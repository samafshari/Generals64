...
// Shroud texture stays bound at t3 from TerrainRenderer::Render() across draw calls.
static void BindOverlayTexturesForGroundPass()
{
	// Nothing to bind — shroud persists from terrain pass.
}
...
void RenderOccludedSilhouettesDX11()
{
	...
	// test GREATER + alpha blend + unlit shader.
	...
	while (draw)
	{
		...
		// render object through ModelRenderer. The bound silhouette state
		// means the mesh draws as a flat colored fill only on pixels
		// where it's behind another mesh.
		for (DrawModule** dm = draw->getDrawModules(); *dm; ++dm)
		{
			ObjectDrawInterface* odi = (*dm)->getObjectDrawInterface();
			if (!odi)
...
void D3D11ParticleSystemManager::doParticles(RenderInfoClass &rinfo)
{
	...
	for (auto& pSystem : allSystems)
	{
		...
		// into ALPHA loses darkening particles (which need DEST*SRC darkening) and
...
// following the same pattern as scorch marks.
////////////////////////////////////////////////////////////////////////////////
