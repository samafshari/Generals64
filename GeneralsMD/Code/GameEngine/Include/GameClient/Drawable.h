...
enum DrawableStatus : DrawableStatusBits
{
...
public:
  ...
  //	inline UnsignedByte getFullyObscuredByShroudWithCheatSpy() {return (UnsignedByte)m_drawableFullyObscuredByShroud | 128;}//8 looks like a zero in most fonts
	...
	const Matrix3D *getPrevTransformMatrix() const;	///< return previous logic frame's world transform (for visual interpolation)
	UnsignedInt getPrevTransformFrame() const;		///< logic frame when prev-transform was captured (delegates to Object's when present)
