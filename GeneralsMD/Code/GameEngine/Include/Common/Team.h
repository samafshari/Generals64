...
public:
	...
	*/
	void setEnteredExited() {m_enteredOrExited = true;}

	/**
		Did a team member enter or exit a trigger area.
	*/
	Bool didEnterOrExit() {return m_enteredOrExited;}
	...
	*/
	...
	// ------------------------------------------------------------------------
	// Group-dispatch primitives. Each wraps getTeamAsAIGroup + the matching
	// AIGroup::group* call, so team orders route through the C&C3-style
	// shared-path + lane-offset system and never devolve into a per-member
	// Pathfinder::findPath loop. Use these from AIPlayer, ScriptActions, or
	// anywhere the engine wants to move/attack an entire team.
	// ------------------------------------------------------------------------
	void groupMoveTo( const Coord3D *pos, CommandSourceType cmdSource );
	void groupAttackMoveTo( const Coord3D *pos, Int maxShotsToFire, CommandSourceType cmdSource );
	...
	void groupAttackArea( const PolygonTrigger *area, CommandSourceType cmdSource );
