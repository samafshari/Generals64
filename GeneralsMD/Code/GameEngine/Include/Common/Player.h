...
public:
	...
	/// Cosmetic shader effect id (0 = stock). Populated from the owning
	/// GameSlot via TheKey_playerShaderId at Player::init. The render
	/// path may override with a fresher value from CosmeticsCache (pushed
	/// by the relay on profile changes); this member is the lobby-time
	/// snapshot used as a fallback when the cache is cold.
	Int  getShaderId() const    { return m_shaderId; }
	void setShaderId(Int id)    { m_shaderId = id; }

	/// Inspector / tooling hook: override the player's unit color at
	/// runtime. Safe because the W3D render path reads getPlayerColor()
	/// every frame when tinting HOUSECOLOR meshes (ModelRenderer.cpp,
	/// W3DScene.cpp), so the change propagates to every owned model
	/// on the NEXT frame with no manual re-bind. The alpha byte is
	/// always forced to 0xFF so downstream bit-unpack code still works.
	void setPlayerColor(Color c) { m_color = c | 0xff000000; m_nightColor = m_color; }
...
private:
	...
	Int													m_shaderId = 0;	///< Cosmetic shader effect (ShaderEffects id). 0 = stock.
