...
public:
	...
	/**Re-apply the current LOD level unconditionally (bypasses setStaticLODLevel's
	   "same level already applied" early-out). Needed for the max-quality hardcode
	   path: checkbox-driven writes to TheWritableGlobalData must be reset back to
	   the LOD preset + max override whenever the options dialog closes, even
	   when the user hasn't changed the detail combo box. */
	void reapplyStaticLODLevel() { applyStaticLODLevel(m_currentStaticLOD); }
