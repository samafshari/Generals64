...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
enum ThingTemplateAudioType : Int
{
...
public:
	...
	*/
	...
	void friend_addDefaultAIModuleInfo();
  ...
  // True when the cap comes from the per-game "superweapon restriction" knob
  // (the LAN options dropdown), in which case Player::canBuildMoreOfType
  // counts the team's combined ownership instead of just the local player's.
