...
//		TheShell->hideShell();
		...
		// Render cap decision is the user's (see LANAPICallbacks.cpp for rationale):
		// now that render is accumulator-decoupled from the network lockstep tick,
		// an uncapped render path on a 60 Hz monitor tears heavily with
		// DXGI ALLOW_TEARING and looks like mesh flashing on static buildings.
