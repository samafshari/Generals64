...
// Inspector hook: feed the Destruction Timeline panel one record per
// dying object so the gantt chart can show who-lost-what-when. Header
// is engine-type-free so this drag-in is cheap and the engine doesn't
// learn anything about ImGui.
#include "Inspector/DestructionTimeline.h"
...
//=============================================================================
static constexpr int BOOBY_TRAP_SCAN_RANGE = 25;
...
void Object::onDie( DamageInfo *damageInfo )
{
	...
	// Inspector Destruction Timeline: record one entry per death so the
	// gantt panel can plot it. Cheap (one mutex'd push to a fixed ring)
	// and unconditional — the panel snapshots the ring whenever it's
	// opened, so the data is there even if the user opens the inspector
	// after the fight is over.
	{
		Player* owner = getControllingPlayer();
		const ThingTemplate* tmpl = getTemplate();
		const char* tmplName = (tmpl && tmpl->getName().str()) ? tmpl->getName().str() : "<unknown>";
		const int  playerIdx = owner ? (int)owner->getPlayerIndex() : -1;
		const unsigned colorARGB = owner ? (unsigned)owner->getPlayerColor() : 0u;
		const char* sideName = "";
		if (owner)
		{
			AsciiString s = owner->getSide();
			if (s.str() && *s.str()) sideName = s.str();
		}
		// Coarse kind classification — picks the most specific bucket
		// the object matches first, so a "vehicle aircraft" lands in
		// AIRCRAFT rather than VEHICLE. Used by the panel for tooltips
		// only; storage cost is one int.
		int kindMask = 0;
		if      (isKindOf(KINDOF_AIRCRAFT))  kindMask = 4;
		else if (isKindOf(KINDOF_STRUCTURE)) kindMask = 2;
		else if (isKindOf(KINDOF_INFANTRY))  kindMask = 3;
		else if (isKindOf(KINDOF_VEHICLE))   kindMask = 1;
		Inspector::Destruction::RecordDeath(
			playerIdx,
			tmplName,
			colorARGB,
			sideName,
