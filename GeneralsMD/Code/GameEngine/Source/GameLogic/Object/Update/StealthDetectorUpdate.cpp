...
UpdateSleepTime StealthDetectorUpdate::update()
{
  ...
  const Player *localPlayer = rts::getObservedOrLocalPlayer();
	...
	// Adaptive sleep: when no stealth units were found, sleep 3x longer
	// since there's nothing to track. This significantly reduces CPU load
	// for maps with many detectors but few stealth units.
	if (!foundSomeone)
		return UPDATE_SLEEP(data->m_updateRate * 3);
