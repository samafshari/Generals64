...
void LaserUpdate::clientUpdate()
{
	...
	if( draw )
	{
		// TEMP: keep laser drawables centered on the beam every frame so region-based
		// visibility tests cannot cull them while we debug beam rendering.
		Coord3D posToUse;
		posToUse.set( &m_startPos );
		posToUse.add( &m_endPos );
		posToUse.scale( 0.5f );
...
void LaserUpdate::initLaser( const Object *parent, const Object *target, const Coord3D *startPos, const Coord3D *endPos, AsciiString parentBoneName, Int sizeDeltaFrames )
{
	...
	// TEMP: always center the drawable on the beam. Parent-position placement can
	// push laser drawables outside the region iteration used by the DX11 renderer.
	// Once laser visibility is solid again we can restore the original conditions.
	Coord3D posToUse;
	posToUse.set( &m_startPos );
	posToUse.add( &m_endPos );
	posToUse.scale( 0.5f );
