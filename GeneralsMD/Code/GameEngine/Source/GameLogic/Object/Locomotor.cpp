...
// Fast atan2 approximation — polynomial, max error ~0.003 radians (~0.17 degrees).
// Replaces expensive CRT atan2 in locomotor hot paths. Self-correcting since
// path following snaps units back to path every frame.
static inline Real fastAtan2(Real y, Real x) {
	Real absX = fabsf(x), absY = fabsf(y);
	if (absX < 1e-10f && absY < 1e-10f) return 0.0f;
	Real a = (absX < absY) ? absX / absY : absY / absX;
	Real s = a * a;
	Real r = ((-0.0464964749f * s + 0.15931422f) * s - 0.327622764f) * s * a + a;
...
//	DEBUG_ASSERTLOG(obj->getID() != TheObjectIDToDebug, ("locoUpdate_moveTowardsPosition %f %f %f (dtg %f, spd %f), speed %f (%f)",goalPos.x,goalPos.y,goalPos.z,onPathDistToGoal,desiredSpeed,physics->getSpeed(),physics->getForwardSpeed2D()));
	...
	Real dist = 0.0f;
	Real onPathDistSqr = onPathDistToGoal * onPathDistToGoal;
	if (distSqr > onPathDistSqr)
	{
		dist = sqrtf(distSqr);
		if (!obj->isKindOf(KINDOF_PROJECTILE) && distSqr > 4.0f * onPathDistSqr)
		{
...
//			m_template->m_appearance == LOCO_HOVER ||
	...
	#define MIN_VEL (PATHFIND_CELL_SIZE_F/(LOGICFRAMES_PER_SECOND))
		...
		else
		{
			...
			// Normalize.  Lazily compute dist if it wasn't computed above.
			if (dist == 0.0f) dist = sqrtf(distSqr);
			if (dist > 0.001f)
			{
...
//	Real relAngle = ThePartitionManager->getRelativeAngle2D( obj, &goalPos );
//	Real desiredAngle = angle + relAngle;
	Real desiredAngle = fastAtan2(goalPos.y - obj->getPosition()->y, goalPos.x - obj->getPosition()->x);
																											 ...
																											 Real maxTurnRate, Real *relAngle)
{
	...
	if (offset != 0.0f)
	{
		...
		Real desiredAngle = fastAtan2(dy, dx);
		...
		// back-calculation.
		Real angleDesiredForTurnPos = fastAtan2(desiredPos.y - turnPos.y, desiredPos.x - turnPos.x);
