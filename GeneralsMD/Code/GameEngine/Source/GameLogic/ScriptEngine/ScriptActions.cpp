...
void ScriptActions::doCreateReinforcements(const AsciiString& team, const AsciiString& waypoint)
{
	...
	if (theTeam)
	{
		if (transport)
		{
			// Residual non-transportable units all head to the same
			// destination and would otherwise each pay a per-unit A*. Collect
			// them into a dedicated AIGroup (excluding transports, which keep
			// their distinct aiMoveToAndEvacuate state below) so we pay one
			// shared path + lane-offset fan-out for the whole residual set.
			AIGroupPtr residualGroup = TheAI->createGroup();
			for (DLINK_ITERATOR<Object> iter = theTeam->iterate_TeamMemberList(); !iter.done(); iter.advance())
			{
				Object *obj = iter.cur();
				if (!obj)
				...
				{
					// Non-transportable: reset locomotor now, defer the move
					// dispatch so we can batch it through residualGroup below.
					if( !obj->isDisabledByType( DISABLED_HELD ) )
					{
						if( ai )
						{
							ai->chooseLocomotorSet( LOCOMOTORSET_NORMAL );
							if ( residualGroup )
								residualGroup->add( obj );
						}
					}
				}
			}
			if ( residualGroup && residualGroup->getCount() > 0 )
			{
				residualGroup->groupMoveToPosition( &destination, /*addWaypoint=*/ false, CMD_FROM_SCRIPT );
			}
		}
		else
		{
