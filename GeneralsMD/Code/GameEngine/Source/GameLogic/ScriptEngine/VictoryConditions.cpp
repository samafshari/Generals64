...
void VictoryConditions::update()
{
	...
	if (!m_singleAllianceRemaining)
	{
		if (!multipleAlliancesExist())
		{
			...
			if (victoriousPlayer)
			{
				...
				// Telemetry: if the local player is on the winning alliance
				// this is a game_won for them. Otherwise they'll pick up
				// game_lost via the defeat branch below.
				if (TheGameTelemetry && !m_isObserver
				    && m_localSlotNum >= 0 && m_localSlotNum < MAX_PLAYER_COUNT
				    && m_isVictorious[m_localSlotNum])
				{
					TheGameTelemetry->onGameWon();
				}
			}
		}
	}
	...
	if (!m_localPlayerDefeated && !m_isObserver)
	{
		...
		if (hasSinglePlayerBeenDefeated(localPlayer))
		{
			...
			// Telemetry: local player just lost. The won-branch above only
			// catches alliance victory; a mid-game elimination comes here.
			if (TheGameTelemetry)
				TheGameTelemetry->onGameLost();
		}
	}
}
