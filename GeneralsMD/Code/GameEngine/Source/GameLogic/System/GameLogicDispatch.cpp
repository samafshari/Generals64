...
void GameLogic::logicMessageDispatcher( GameMessage *msg, void *userData )
{
...
#endif
	// Defensive guards: crash reports have shown this function being entered
	// with a null `this` during late shutdown / early init paths (accessing
	// m_gameMode at offset 0x88 on null → 0xC0000005). The dispatch normally
	// goes through `TheGameLogic->processProgress() → this->logicMessageDispatcher()`
	// but something in the GUI update path has been observed to reach this
	// function with a stale GameLogic pointer. Bail out cleanly instead of
	// crashing — release-mode stack traces are too noisy to pin the real
	// caller, so treat this as a safety belt.
	if (TheGameLogic == nullptr) return;
	if (msg == nullptr) return;
	if (ThePlayerList == nullptr) return;
	...
	switch( msgType )
	{
		...
		case GameMessage::MSG_NEW_GAME:
		{
			...
			{
				Int logicFPS = msg->getArgument( 3 )->integer;
				if (logicFPS < LOGICFRAMES_PER_SECOND || logicFPS > 120)
					logicFPS = LOGICFRAMES_PER_SECOND;
				DEBUG_LOG(("Setting logic FPS to %d, render FPS uncapped", logicFPS));
