...
// Dummy implementation for externalAddTree
void externalAddTree(Coord3D location, Real scale, Real angle, AsciiString name)
{
...
static Bool isShellMapTraceEnabled()
{
	return FALSE;
}

static void appendShellMapTrace(const char *format, ...)
{
...
void setFPMode()
{
  ...
  // system.
	...
	//
#ifdef _M_IX86
	// x87 FPU: set single-precision, round-to-nearest for deterministic sim
	_fpreset();
...
static void checkForDuplicateColors( GameInfo *game )
{
	...
	for (i=MAX_SLOTS-1; i>=0; --i)
	{
		...
		// Slot colors are raw 0x00RRGGBB since the launcher revamp, not palette
		// indices (see memory/slot_color_rgb_schema.md). Compare the raw value
		// against the other slots via isColorTaken, which also stores raw RGB.
		slot->setColor(-1);
		if (!game->isColorTaken(slotColor, i))
		{
		...
		{
			DEBUG_LOG(("Clearing color 0x%06X for player %d", slotColor, i));
		}
	}
}
...
#define MORE_RANDOM
	...
	for (i=0; i<MAX_SLOTS; ++i)
	{
		...
		// Slot colors are raw 0x00RRGGBB since the launcher revamp (see
		// memory/slot_color_rgb_schema.md). Only -1 is the "random, pick one"
		// sentinel; any other value is a real RGB and stays as-is.
		Int slotColor = slot->getColor();
		if (slotColor == -1)
		{
			Int numColors = TheMultiplayerSettings->getNumColors();
			Int pickedRgb = -1;
			// Bounded attempts: if every palette entry is taken we fall back
			// to whatever we drew last (better than spinning forever).
			for (Int attempt = 0; attempt < 64 && numColors > 0; ++attempt)
			{
				Int paletteIdx = GameLogicRandomValue(0, numColors - 1);
				MultiplayerColorDefinition *def = TheMultiplayerSettings->getColor(paletteIdx);
				if (!def)
					continue;
				Int rgb = MultiplayerSettings::packSlotColor((UnsignedInt)def->getColor());
				if (!game->isColorTaken(rgb, i))
				{
					pickedRgb = rgb;
					break;
				}
				pickedRgb = rgb; // last-ditch fallback
			}
			if (pickedRgb != -1)
			{
				DEBUG_LOG(("Setting random color for slot %d to 0x%06X", i, pickedRgb));
				slot->setColor(pickedRgb);
			}
		}
	}
}
...
void GameLogic::startNewGame( Bool loadingSaveGame )
{
  ...
  if ( !loadingSaveGame )
  {
	...
	// Set logic time scale based on game type.
	//
	// For skirmish and multiplayer, honor the host / user pick that arrived
	// either via MSG_NEW_GAME arg[3] (SkirmishGameOptionsMenu / external
	// launcher — already applied to TheFramePacer by logicMessageDispatcher
	// before we get here) or via TheGameInfo::getGameFps (set by the LAN /
	// Skirmish game-speed combo boxes). Previously this site hard-coded 70
	// for every skirmish / MP session, silently discarding the combo-box
	// selection and making the in-game "game speed" option a no-op for
	// single-player skirmish.
	//
	// Campaign and challenge keep the original 30 Hz — those game modes
	// weren't rebalanced for higher logic rates and cinematic scripts rely
	// on 30 Hz timing.
	if (!loadingSaveGame)
	{
		Int logicFps = LOGICFRAMES_PER_SECOND; // default 30
		if (TheGameInfo)
		{
			if (TheGameEngine->isMultiplayerSession() || isSkirmishOrSkirmishReplay)
			{
				// Prefer GameInfo (persistent across the session) then the
				// pre-applied pacer value (set by MSG_NEW_GAME dispatch), else
				// fall back to the pre-patch 70 default to preserve historical
				// skirmish feel.
				Int pick = TheGameInfo->getGameFps();
				if (pick < LOGICFRAMES_PER_SECOND || pick > 120)
					pick = TheFramePacer ? TheFramePacer->getLogicTimeScaleFps() : 70;
				if (pick < LOGICFRAMES_PER_SECOND || pick > 120)
					pick = 70;
				logicFps = pick;
			}
			// else campaign or challenge: keep 30fps
		}
		// else shell map: keep 30fps

		TheFramePacer->setLogicTimeScaleFps(logicFps);
		TheFramePacer->enableLogicTimeScale(logicFps != LOGICFRAMES_PER_SECOND);
	}
	...
	appendShellMapTrace("GameLogic: post loadMap progress=%d", LOAD_PROGRESS_POST_LOAD_MAP);
...
/*
			...
			*/
			...
			// Slot color is raw 0x00RRGGBB (post launcher-revamp), NOT a
			// palette index. Passing it to getColor(Int) fails the bounds
			// check, returns nullptr, and the next ->getColor() null-derefs
			// — crashes at countdown-end → load-screen handoff. Funnel
			// through MultiplayerSettings::resolveSlotColor which also
			// forces alpha to 0xFF and maps the -1 random sentinel to white.
			// See memory/slot_color_rgb_schema.md.
			Color playerColorARGB = MultiplayerSettings::resolveSlotColor(slot->getColor());
			Color playerNightColorARGB = MultiplayerSettings::resolveSlotNightColor(slot->getColor());
			d.setInt(TheKey_playerColor, (Int)playerColorARGB);
			d.setInt(TheKey_playerNightColor, (Int)playerNightColorARGB);
...
*/
	...
	appendShellMapTrace("GameLogic: post side list init progress=%d", LOAD_PROGRESS_POST_SIDE_LIST_INIT);
	...
	appendShellMapTrace("GameLogic: post script engine newMap progress=%d", LOAD_PROGRESS_POST_SCRIPT_ENGINE_NEW_MAP);
	...
	appendShellMapTrace("GameLogic: post victory setup progress=%d", LOAD_PROGRESS_POST_VICTORY_CONDITION_SETUP);
	...
	appendShellMapTrace("GameLogic: post victory condition progress=%d", LOAD_PROGRESS_POST_VICTORY_CONDITION_SET_VICTORY_CONDITION);
	...
	appendShellMapTrace("GameLogic: post ghost object reset progress=%d", LOAD_PROGRESS_POST_GHOST_OBJECT_MANAGER_RESET);

	// update the terrain logic now that all is loaded
	appendShellMapTrace("GameLogic: before terrain newMap");
	TheTerrainLogic->newMap( loadingSaveGame );

	// update the loadscreen
	updateLoadProgress(LOAD_PROGRESS_POST_TERRAIN_LOGIC_NEW_MAP);
	appendShellMapTrace("GameLogic: post terrain newMap progress=%d", LOAD_PROGRESS_POST_TERRAIN_LOGIC_NEW_MAP);
	...
	appendShellMapTrace("GameLogic: post bridge load progress=%d", LOAD_PROGRESS_POST_BRIDGE_LOAD);
	...
	appendShellMapTrace("GameLogic: post pathfinder newMap progress=%d", LOAD_PROGRESS_POST_PATHFINDER_NEW_MAP);
	...
	else
	{
		...
		Int shellTraceObjectIndex = 0;
		appendShellMapTrace("GameLogic: begin map object load");
		for (MapObject *pMapObj = MapObject::getFirstMapObject(); pMapObj; pMapObj = pMapObj->getNext())
		{
			...
			++shellTraceObjectIndex;
			appendShellMapTrace(
				"GameLogic: object #%d mapObject=%s template=%s",
				shellTraceObjectIndex,
				pMapObj->getName().str(),
			...
			if (thingTemplate->isKindOf(KINDOF_OPTIMIZED_TREE)) {
				appendShellMapTrace("GameLogic: object #%d createOptimizedTree", shellTraceObjectIndex);
				createOptimizedTree(thingTemplate, &pos, angle);
				appendShellMapTrace("GameLogic: object #%d createOptimizedTree done", shellTraceObjectIndex);
				continue;
			}
			...
			if (isProp || (isFluff && forceFluffToProp)) {
				appendShellMapTrace("GameLogic: object #%d addProp", shellTraceObjectIndex);
				TheTerrainVisual->addProp(thingTemplate, &pos, angle);
				appendShellMapTrace("GameLogic: object #%d addProp done", shellTraceObjectIndex);
				continue;
			}
#endif
			...
			// create new object in the world
			appendShellMapTrace("GameLogic: object #%d newObject begin", shellTraceObjectIndex);
			Object *obj = TheThingFactory->newObject( thingTemplate, team ); //, OBJECT_STATUS_LOADING_FROM_MAP );
			appendShellMapTrace("GameLogic: object #%d newObject end obj=%p", shellTraceObjectIndex, obj);
			if( obj )
			{
				...
				// in the map object properties.
				// update this object instance with properties from the map object
				appendShellMapTrace("GameLogic: object #%d updateObjValues begin", shellTraceObjectIndex);
				obj->updateObjValuesFromMapProperties( pMapObj->getProperties() );
				appendShellMapTrace("GameLogic: object #%d updateObjValues end", shellTraceObjectIndex);
				...
				// Since the team now has members, activate it.
				team->setActive();
				appendShellMapTrace("GameLogic: object #%d addObjectToPathfindMap begin", shellTraceObjectIndex);
				TheAI->pathfinder()->addObjectToPathfindMap( obj );
				appendShellMapTrace("GameLogic: object #%d addObjectToPathfindMap end", shellTraceObjectIndex);
		...
		appendShellMapTrace("GameLogic: end map object load total=%d", shellTraceObjectIndex);
	...
	appendShellMapTrace("GameLogic: post initial network buildings progress=%d", LOAD_PROGRESS_POST_INITIAL_NETWORK_BUILDINGS);
	...
	appendShellMapTrace("GameLogic: post preload assets progress=%d", LOAD_PROGRESS_POST_PRELOAD_ASSETS);
	...
	appendShellMapTrace("GameLogic: post starting camera progress=%d", LOAD_PROGRESS_POST_STARTING_CAMERA);
...
void GameLogic::processDestroyList()
{
	...
	/*
		Batch scan: instead of scanning m_sleepyUpdates once per dying object (O(n*m) with m
		vector passes), do a single pass collecting modules for all dying objects at once.
		This is critical for mods with many projectiles/effects dying per frame.
	*/

	const Int numToDestroy = (Int)m_objectsToDestroy.size();
	...
	if (numToDestroy > 0)
	{
		...
		{
			Object* modObj = (*it)->friend_getObject();
			Bool found = FALSE;
			for (ObjectPointerListIterator dit = m_objectsToDestroy.begin(); dit != m_objectsToDestroy.end(); ++dit)
			{
				if (*dit == modObj) { found = TRUE; break; }
			}
			if (found)
		...
		const Int MAX_SUO = 4096;
		UpdateModulePtr sleepyUpdatesToErase[MAX_SUO];
		Int numSUO = 0;

		// Single pass through the entire heap, matching modules against all dying objects
		for (std::vector<UpdateModulePtr>::iterator it2 = m_sleepyUpdates.begin(); it2 != m_sleepyUpdates.end(); ++it2)
		{
			const Object* modObj = (*it2)->friend_getObject();
			for (ObjectPointerListIterator dit = m_objectsToDestroy.begin(); dit != m_objectsToDestroy.end(); ++dit)
			{
				if (*dit == modObj)
				{
					if (numSUO < MAX_SUO)
						sleepyUpdatesToErase[numSUO++] = *it2;
					break;
				}
			}
		}

		// Erase collected modules from the heap
		for (--numSUO; numSUO >= 0; --numSUO)
		{
			Int idx = sleepyUpdatesToErase[numSUO]->friend_getIndexInLogic();
			DEBUG_ASSERTCRASH(m_sleepyUpdates[idx] == sleepyUpdatesToErase[numSUO], ("Hmm, expected update mismatch here"));
			eraseSleepyUpdate(idx);
			DEBUG_ASSERTCRASH(sleepyUpdatesToErase[numSUO]->friend_getIndexInLogic() == -1, ("Hmm, expected index to be -1 here"));
		}
	}
...
void GameLogic::update()
{
...
#ifdef ALLOW_NONSLEEPY_UPDATES
	{
		for (std::list<UpdateModulePtr>::const_iterator it = m_normalUpdates.begin(); it != m_normalUpdates.end(); ++it)
		{
			...
			if (!u->friend_getObject()) continue;
	...
	int sleepySkipCount = 0;
	{
		...
		while (!m_sleepyUpdates.empty())
		{
			...
			{
				sleepySkipCount = (int)m_sleepyUpdates.size() - sleepyUpdateCount;
				break;
			}
			++sleepyUpdateCount;
...
//	m_objHash[ obj->getID() ] = obj;
	...
	{
		size_t newSize = m_objVector.empty() ? 2 : m_objVector.size() * 2;
		if( newSize <= newID )
			newSize = static_cast<size_t>(newID) + 1;
...
{
	appendShellMapTrace("GameLogic: sendObjectCreated begin template=%s", obj->getTemplate()->getName().str());
	Drawable *draw = TheThingFactory->newDrawable(obj->getTemplate());
	appendShellMapTrace("GameLogic: sendObjectCreated newDrawable end template=%s drawable=%p", obj->getTemplate()->getName().str(), draw);
...
/// @todo COLIN ... shouldn't we have a check here for existing drawable!!!!!
	...
	appendShellMapTrace("GameLogic: sendObjectCreated bind end template=%s", obj->getTemplate()->getName().str());
