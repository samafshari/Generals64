...
{
	USE_PERF_TIMER(AIIdleState)
	LIVE_PERF_SCOPE("AIIdleState::update");

	doInitIdleState();

	// Per-unit deterministic phase jitter so units that entered AI_IDLE in the
	// same frame stop colliding on every scan after their initial random
	// offset is consumed. The original code only randomized m_initialSleepOffset
	// in onEnter and then reset it to 0, so a wave of N units spawned together
	// (e.g. an AI player's initial dozer + workers, a transport unloading,
	// reinforcements arriving, units reverting from a Hold-Position group)
	// would collide on findClosestEnemy / findClosestRepulsor every
	// IDLE_COUNTDOWN_DELAY frames forever — a hard rendezvous on the partition
	// manager that scales O(N) per AI tick.
	//
	// Phase jitter is a function of objectID, which is deterministic across
	// clients and (critically) does NOT consume the lockstep RNG stream, so
	// it's replay- and network-sync-safe. The /2 keeps the worst-case scan
	// period at 1.5 * IDLE_COUNTDOWN_DELAY (3 sec) — units stay reactive while
	// load gets spread across IDLE_COUNTDOWN_DELAY/2 frame buckets instead of 1.
	const UnsignedInt phaseJitter =
		(UnsignedInt)(getMachineOwner()->getID() % (IDLE_COUNTDOWN_DELAY / 2));
	UnsignedInt timeToSleep = IDLE_COUNTDOWN_DELAY + m_initialSleepOffset + phaseJitter;
...
{
	USE_PERF_TIMER(AIAttackState)
	LIVE_PERF_SCOPE("AIAttackState::update");
	// if we've met the conditions specified by m_attackParameters, we consider ourselves "successful."
	if (m_attackParameters && m_attackParameters->shouldExit(getMachine()))
	{
		return STATE_SUCCESS;
