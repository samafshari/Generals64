	...
	for( BuildListInfo *info = m_player->getBuildList(); info; info = info->getNext() )
	{
		...
		if (curGatherers>=desiredGatherers) {
			...
			if (supplyCenter) {
				...
				// Cache the nearest SUPPLY_SOURCE per supply center. Warehouses/piles are static — the
				// identity of the closest non-enemy source only changes when one is destroyed, or drained
				// to empty. Validate cached hit; full rescan every SUPPLY_SOURCE_CACHE_TTL frames.
				static const UnsignedInt SUPPLY_SOURCE_CACHE_TTL = 30; // ~1 s of logic frames
				const UnsignedInt nowFrame = TheGameLogic->getFrame();
				ClosestObjectCacheEntry& sscEntry = m_supplySourceCache[supplyCenter->getID()];
				Object *supplySource = nullptr;
				if (sscEntry.result != INVALID_ID && (nowFrame - sscEntry.frame) < SUPPLY_SOURCE_CACHE_TTL) {
					supplySource = TheGameLogic->findObjectByID(sscEntry.result);
				}
				if (!supplySource) {
					...
					supplySource = ThePartitionManager->getClosestObject(&center, radius, FROM_BOUNDINGSPHERE_2D, filters);
					sscEntry.result = supplySource ? supplySource->getID() : INVALID_ID;
					sscEntry.frame = nowFrame;
				}
				if (!supplySource) {
					// No supplies.
					continue;
...
Bool AIPlayer::isLocationSafe(const Coord3D *pos, const ThingTemplate *tthing )
{
	...
	// Expansion scouting is strategic, not tactical — safe to cache the full-radius scan for
	// a short window. Key hashes (quantized cell pos, template id). TTL kept short enough that
	// enemy arrival is still detected within ~1 s.
	static const UnsignedInt LOCATION_SAFE_CACHE_TTL = 30; // ~1 s
	const UnsignedInt nowFrameLS = TheGameLogic->getFrame();
	const Int qx = REAL_TO_INT_FLOOR(pos->x / PATHFIND_CELL_SIZE_F);
	const Int qy = REAL_TO_INT_FLOOR(pos->y / PATHFIND_CELL_SIZE_F);
	const UnsignedInt64 cacheKey =
		(((UnsignedInt64)(UnsignedInt)qx) & 0xFFFFFFULL)
	...
	const Bool safe = (enemy == nullptr);
	LocationSafeCacheEntry& lsEntry = m_locationSafeCache[cacheKey];
	lsEntry.result = safe;
	lsEntry.frame = nowFrameLS;
	return safe;
}
...
{
	LIVE_PERF_SCOPE("AIPlayer::findSupplyCenter");
	...
	do {
		for( obj = TheGameLogic->getFirstObject(); obj; obj = obj->getNextObject() )
		{
			...
			if( warehouseModule )	{
				...
				// Cache "do I already have a CASH_GENERATOR near this supply source?" per source ID.
				// The answer only flips when we build/lose a cash gen; a short TTL catches that at
				// acceptable lag for expansion planning (not a combat-critical query).
				static const UnsignedInt OWN_CASHGEN_CACHE_TTL = 60; // ~2 s
				const UnsignedInt nowFrame2 = TheGameLogic->getFrame();
				ClosestObjectCacheEntry& ogEntry = m_ownCashGenNearSourceCache[obj->getID()];
				Object *supplyCenter = nullptr;
				Bool cachedHit = false;
				if ((nowFrame2 - ogEntry.frame) < OWN_CASHGEN_CACHE_TTL && ogEntry.frame != 0) {
					if (ogEntry.result != INVALID_ID) {
						supplyCenter = TheGameLogic->findObjectByID(ogEntry.result);
						if (supplyCenter) {
							cachedHit = true;
						}
					} else {
						// Cache says "no cash gen near this source" — trust it for the TTL window.
						cachedHit = true;
					}
				}
				if (!cachedHit) {
					...
					supplyCenter = ThePartitionManager->getClosestObject(&center, radius, FROM_BOUNDINGSPHERE_2D, filters);
					ogEntry.result = supplyCenter ? supplyCenter->getID() : INVALID_ID;
					ogEntry.frame = nowFrame2;
				}
				if (supplyCenter) {
...
//		WorkOrder *orders = nullptr;
			...
			if (thing)
			{
				...
				while (count>0) {
					...
					if (unit)
					{
						...
						// Don't dispatch per-unit here — we batch once at the end
						// through Team::groupMoveTo for a single shared A*.
					} else {
						break;
					}
					count--;
		...
		// One shared-path dispatch for every freshly-recruited member, rather
		// than N per-unit aiMoveToPosition calls that each pay full Pathfinder
		// cost. theTeam->m_memberList was populated by the setTeam() calls above.
		if (unitsRecruited > 0)
		{
			Coord3D destination = teamProto->getTemplateInfo()->m_homeLocation;
			theTeam->groupMoveTo( &destination, CMD_FROM_AI );
		}
		if (unitsRecruited>0)
		{
			/* We have something to build. */
		...
		for( WorkOrder *order = team->m_workOrders; order; order = order->m_next )
		{
			...
			while (order->isWaitingToBuild()) {
				...
				if (unit)
				{
					...
					// Move dispatch is batched via Team::groupMoveTo after the
					// inner while-recruit loop — see below. Single-unit idle
					// still fires per-unit since it doesn't hit the pathfinder.
					if (!hasHome) {
						AIUpdateInterface *ai = unit->getAIUpdateInterface();
			...
			// Batch the move for everyone we just recruited. Path computed once,
			// fanned out via lane offsets. A cliff-high tank team recruited
			// across the base now pays one A* instead of tens.
			if (hasHome) {
				team->m_team->groupMoveTo( &home, CMD_FROM_AI );
			}
		...
		for ( DLINK_ITERATOR<TeamInQueue> iter = iterate_TeamReadyQueue(); !iter.done(); iter.advance())
		{
			...
			if (allIdle) {
				if (!team->m_sentToStartLocation) {
					...
					// Un-abandoned from the original drafted-but-commented block:
					// send the freshly-built team from its rally position to the
					// prototype home location before setActive fires its OnCreate
					// script. One shared A* via Team::groupMoveTo instead of N
					// per-member aiMoveToPosition calls. Using groupMoveTo (not
					// groupTightenToPosition) avoids the tighten-into-a-pile
					// behavior that likely caused the original author to shelve
					// this code. The `continue` defers setActive by one loop
					// iteration, which is intentional — the team activates once
					// it re-enters this loop idle at its new home position.
					if (team->m_team->getPrototype()->getTemplateInfo()->m_hasHomeLocation &&
							!team->m_reinforcement) {
						Coord3D destination = team->m_team->getPrototype()->getTemplateInfo()->m_homeLocation;
						team->m_team->groupMoveTo( &destination, CMD_FROM_AI );
						team->m_frameStarted = TheGameLogic->getFrame();
						continue;
					}
				}
...
void AIPlayer::update()
{
	...
	// doBaseBuilding self-throttles to 2s via m_buildDelay, doTeamBuilding to
	// 5s via m_teamDelay, updateBridgeRepair to 1s via m_bridgeTimer). The team
	// checks are pure state-transition probes — "is this team's rally complete?",
	// "is this team's build done?" — and are tolerant of being run on a coarser
	// cadence: a team launch delayed by ≤5 logic frames (~166 ms) is invisible
	// against the 60-second team build budget the engine already accepts. Run
	// them every 5 logic frames, phased by player index so 8 AI players don't
	// all hit their team checks on the same frame. Determinism preserved:
	// getFrame() is the lockstep frame, getPlayerIndex() is fixed at game start.
...
{
	LIVE_PERF_SCOPE("AIPlayer::findDozer");
	// Add any factories placed to the build list.
	Object *obj;
	Object *dozer = nullptr;
	Bool needDozer = true;
