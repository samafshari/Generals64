			...
			if( currentlyValid )
			{
				if( type == DO_COMMAND || type == EVALUATE_ONLY )
				{
					...
					if( type == DO_COMMAND )
					{
						...
						{ FILE *gdbg = fopen("guard_debug.log", "a"); if (gdbg) { fprintf(gdbg, "  >>> CLEAR FROM: CommandXlat DO_COMMAND for context-command (cmdType=%d)\n", (int)command->getCommandType()); fclose(gdbg); } }
#endif
...
*/
		...
		*/
		...
		//-----------------------------------------------------------------------------------------
		case GameMessage::MSG_META_TOGGLE_PERF_METRICS:
			// F11: show/hide the top-right perf HUD overlay (game timer,
			// render fps, system clock, network latency). Auto-repeat is
			// filtered upstream in MetaEventTranslator::translateGameMessage,
			// so holding F11 won't thrash the flag.
			if (TheInGameUI)
			{
				TheInGameUI->togglePerfMetrics();
			}
			disp = DESTROY_MESSAGE;
			break;
