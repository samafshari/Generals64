...
static Bool isShellMapDrawableTraceEnabled()
{
	return FALSE; // Disabled - writing to 2.6GB log file causes 850ms/frame performance
}

static void appendShellMapDrawableTrace(const char *format, ...)
{
...
Drawable::Drawable( const ThingTemplate *thingTemplate, DrawableStatusBits statusBits )
				: Thing( thingTemplate )
{
	appendShellMapDrawableTrace("Drawable: ctor begin template=%s", thingTemplate ? thingTemplate->getName().str() : "<null>");
  ...
  m_customSoundAmbientInfo = nullptr;
	...
	//
	appendShellMapDrawableTrace("Drawable: registerDrawable begin template=%s", thingTemplate->getName().str());
	TheGameClient->registerDrawable( this );
	appendShellMapDrawableTrace("Drawable: registerDrawable end template=%s", thingTemplate->getName().str());
	...
	appendShellMapDrawableTrace("Drawable: constructDisplayString begin template=%s", thingTemplate->getName().str());
	m_constructDisplayString = TheDisplayStringManager->newDisplayString();
	appendShellMapDrawableTrace("Drawable: constructDisplayString allocated template=%s displayString=%p", thingTemplate->getName().str(), m_constructDisplayString);
	appendShellMapDrawableTrace("Drawable: constructDisplayString setFont begin template=%s font=%s size=%d bold=%d",
	...
	m_constructDisplayString->setFont(TheFontLibrary->getFont(TheInGameUI->getDrawableCaptionFontName(),
																TheGlobalLanguageData->adjustFontSize(TheInGameUI->getDrawableCaptionPointSize()),
																TheInGameUI->isDrawableCaptionBold() ));
	appendShellMapDrawableTrace("Drawable: constructDisplayString setFont end template=%s", thingTemplate->getName().str());
  ...
  m_receivesDynamicLights = TRUE; // a good default... overridden by one of my draw modules if at all
	...
	for (modIdx = 0; modIdx < drawMI.getCount(); ++modIdx)
	{
		...
		if (TheGlobalData->m_useDrawModuleLOD &&
			...
			continue;
		appendShellMapDrawableTrace("Drawable: draw module begin template=%s module=%s", thingTemplate->getName().str(), drawMI.getNthName(modIdx).str());
		*m++ = TheModuleFactory->newModule(this, drawMI.getNthName(modIdx), newModData, MODULETYPE_DRAW);
		appendShellMapDrawableTrace("Drawable: draw module end template=%s module=%s", thingTemplate->getName().str(), drawMI.getNthName(modIdx).str());
	}
	*m = nullptr;
	...
	/// @todo srj -- this is evil, we shouldn't look at the module name directly!
			...
			appendShellMapDrawableTrace("Drawable: client update module begin template=%s module=%s", thingTemplate->getName().str(), cuMI.getNthName(modIdx).str());
			*m++ = TheModuleFactory->newModule(this, cuMI.getNthName(modIdx), newModData, MODULETYPE_CLIENT_UPDATE);
			appendShellMapDrawableTrace("Drawable: client update module end template=%s module=%s", thingTemplate->getName().str(), cuMI.getNthName(modIdx).str());
		}
		*m = nullptr;
	}
	...
	for (i = 0; i < NUM_DRAWABLE_MODULE_TYPES; ++i)
	{
		...
		{
			appendShellMapDrawableTrace("Drawable: module onObjectCreated begin template=%s moduleType=%d", thingTemplate->getName().str(), i + FIRST_DRAWABLE_MODULE_TYPE);
			(*m)->onObjectCreated();
			appendShellMapDrawableTrace("Drawable: module onObjectCreated end template=%s moduleType=%d", thingTemplate->getName().str(), i + FIRST_DRAWABLE_MODULE_TYPE);
		}
	}
...
void Drawable::draw()
{
	// Stealth opacity fade: only process when there's actually opacity to fade
	if ( m_secondMaterialPassOpacity > 0.0f && testTintStatus( TINT_STATUS_FRENZY ) == FALSE )
	{
		Object* obj = getObject();
	...
	Bool isLaserDrawable = FALSE;
	for (DrawModule** dm = getDrawModules(); *dm; ++dm)
	{
		if ((*dm)->getLaserDrawInterface() != nullptr)
		{
			isLaserDrawable = TRUE;
			break;
		}
	}

	// TEMP: let lasers draw through hidden/stealth/shroud rejection while we
	...
	const Matrix3D* currentMtx = getTransformMatrix();
	Matrix3D transformMtx;
	const Real interpFraction = TheGameEngine->getInterpolationFraction();
	// Only lerp when prev is FRESH — captured during the CURRENT logic frame's
	// setter pass. A prev from 2+ ticks ago is stale: the last captured prev
	// represents "state before last motion" but current is "state after last
	// motion" which also equals "current stable state". Lerping between them
	// would re-play the last motion segment on every tick, producing a
	// visible oscillation for units that stopped moving and buildings that
	// were placed once and never touched again. Skip the lerp in that case.
	const UnsignedInt currentLogicFrame = TheGameLogic ? TheGameLogic->getFrame() : 0;
	const Bool prevIsFresh = hasPrevTransform() && getPrevTransformFrame() == currentLogicFrame;
	if (interpFraction > 0.0f && prevIsFresh)
	{
		const Matrix3D* prevMtx = getPrevTransformMatrix();
		Matrix3D::Lerp(*prevMtx, *currentMtx, interpFraction, transformMtx);
	}
	else
	{
		transformMtx = *currentMtx;
	}
...
void Drawable::friend_bindToObject( Object *obj ) ///< bind this drawable to an object ID
{
	...
	{
#ifdef DEBUG_LOGGING
		// DIAG: log up to 400 unique (template, color) bindings so we
		// can match each on-screen unit with its applied team color.
		{
			static std::unordered_set<uint64_t> s_seenBindings;
			Color c = getObject()->getIndicatorColor();
			const char* tname = getObject()->getTemplate() ?
				getObject()->getTemplate()->getName().str() : "(null)";
			uint64_t key = ((uint64_t)(uint32_t)c << 32) | (uintptr_t)tname;
			if (s_seenBindings.size() < 400 && s_seenBindings.insert(key).second)
			{
				const Player* p = getObject()->getControllingPlayer();
				const char* side = (p ? p->getSide().str() : "(null)");
				fprintf(stderr,
					"[Bind] template='%s' color=0x%08X (R=%u G=%u B=%u) side='%s'\n",
					tname,
					(unsigned)c,
					(unsigned)((c >> 16) & 0xFF),
					(unsigned)((c >>  8) & 0xFF),
...
//-------------------------------------------------------------------------------------------------
const Matrix3D *Drawable::getPrevTransformMatrix() const
{
	const Object *obj = getObject();

	if (obj)
		return obj->getPrevTransformMatrix();
	else
		return Thing::getPrevTransformMatrix();
}

//-------------------------------------------------------------------------------------------------
UnsignedInt Drawable::getPrevTransformFrame() const
{
	const Object *obj = getObject();
	return obj ? obj->getPrevTransformFrame() : Thing::getPrevTransformFrame();
}
...
/** Xfer Method
	* Version Info;
	* 1: Initial version
	* 2: Serialize sustain counter as float instead of integer
	*/
// ------------------------------------------------------------------------------------------------
