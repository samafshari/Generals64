...
void InGameUI::update()
{
	...
	if ( m_videoStream && m_videoBuffer )
	{
		if ( m_videoStream->isFrameReady())
		{
			...
			const Int lastFrame = m_videoStream->frameCount() - 1;
			if ( m_videoStream->frameIndex() == 0 || (lastFrame >= 0 && m_videoStream->frameIndex() >= lastFrame) )
			{
				stopMovie();
			}
		}
...
void InGameUI::freeCustomUiResources()
{
	...
	m_gameTimeFrameString = nullptr;
	TheDisplayStringManager->freeDisplayString(m_networkStatsString);
	m_networkStatsString = nullptr;
	TheDisplayStringManager->freeDisplayString(m_frameTimeString);
	m_frameTimeString = nullptr;
...
void InGameUI::postWindowDraw()
{
	...
	// The whole top-right HUD overlay (network latency, render FPS, system
	// clock, game timer) is hidden by default and toggled via F11 ->
	// MSG_META_TOGGLE_PERF_METRICS -> togglePerfMetrics().
	if (m_showPerfMetrics)
	{
...
void InGameUI::refreshCustomUiResources()
{
	...
	refreshSuperweaponAndTimerFonts();
}

// After a resolution change the global font scale
// (TheGlobalLanguageData->adjustFontSize) shifts, but the long-lived
// DisplayStrings owned by SuperweaponInfo / NamedTimerInfo still hold the
// original-resolution GameFont* they were created with. Walk every active
// SuperweaponInfo / NamedTimerInfo and re-apply its font through the
// current adjustFontSize so they snap to the new resolution's font scale.
void InGameUI::refreshSuperweaponAndTimerFonts()
{
	for (Int p = 0; p < MAX_PLAYER_COUNT; ++p)
	{
		for (SuperweaponMap::iterator mapIt = m_superweapons[p].begin();
			 mapIt != m_superweapons[p].end(); ++mapIt)
		{
	...
	for (NamedTimerMapIt it = m_namedTimers.begin(); it != m_namedTimers.end(); ++it)
	{
		NamedTimerInfo* info = it->second;
...
void InGameUI::refreshGameTimeResources()
{
	...
	if (!m_networkStatsString)
	{
		m_networkStatsString = TheDisplayStringManager->newDisplayString();
	}
	m_networkStatsString->setFont(gameTimeFont);

	if (!m_frameTimeString)
	{
		m_frameTimeString = TheDisplayStringManager->newDisplayString();
	}
	m_frameTimeString->setFont(gameTimeFont);
}
...
void InGameUI::drawGameTime()
{
	...
	Int fontHeight = m_gameTimeString->getFont()->height;
	Int nextLineY = m_gameTimePosition.y + fontHeight + 2;

	// Draw network stats (rx/tx/ping) below the game timer in multiplayer games
	if (TheNetwork != nullptr && m_networkStatsString != nullptr)
	{
		Real rxKBps = TheNetwork->getIncomingBytesPerSecond() / 1024.0f;
		Real txKBps = TheNetwork->getOutgoingBytesPerSecond() / 1024.0f;
		UnsignedInt pingMs = (UnsignedInt)(TheNetwork->getRunAhead() * MSEC_PER_LOGICFRAME_REAL);

		UnicodeString netStats;
		netStats.format(L"RX:%.1fK TX:%.1fK %ums", rxKBps, txKBps, pingMs);
		m_networkStatsString->setText(netStats);

		Int horizontalNetOffset = TheDisplay->getWidth() - (Int)m_gameTimePosition.x - m_networkStatsString->getWidth();
		m_networkStatsString->draw(horizontalNetOffset, nextLineY, GameMakeColor(173, 216, 255, 255), m_gameTimeDropColor);
		nextLineY += fontHeight + 2;
	}

	// Frame time measurement and mini graph
	if (m_frameTimeString != nullptr)
	{
		UnsignedInt nowMs = timeGetTime();
		if (m_lastFrameTimeMs != 0)
		{
			Real frameTimeMs = (Real)(nowMs - m_lastFrameTimeMs);
			m_frameTimeHistory[m_frameTimeHistoryIndex] = frameTimeMs;
			m_frameTimeHistoryIndex = (m_frameTimeHistoryIndex + 1) % FRAME_HISTORY_SIZE;
		}
		m_lastFrameTimeMs = nowMs;

		// Compute average frame time from history
		Real sum = 0.0f;
		Real maxFrameTime = 0.0f;
		Int validCount = 0;
		...
		Real avgFrameTime = validCount > 0 ? sum / validCount : 0.0f;
		Real renderFps = TheDisplay->getAverageFPS();

		// Frame time text line
		UnicodeString frameInfo;
		frameInfo.format(L"%.1fms avg:%.1fms max:%.0fms fps:%.0f",
			validCount > 0 ? m_frameTimeHistory[(m_frameTimeHistoryIndex + FRAME_HISTORY_SIZE - 1) % FRAME_HISTORY_SIZE] : 0.0f,
			avgFrameTime, maxFrameTime, renderFps);
		m_frameTimeString->setText(frameInfo);

		Int horizontalTextOffset = TheDisplay->getWidth() - (Int)m_gameTimePosition.x - m_frameTimeString->getWidth();
		m_frameTimeString->draw(horizontalTextOffset, nextLineY, GameMakeColor(220, 220, 180, 255), m_gameTimeDropColor);
		nextLineY += fontHeight + 2;

		// Mini bar graph - each bar is 2px wide, height proportional to frame time
		const Int barWidth = 2;
		const Int graphHeight = 30;
		const Int graphWidth = FRAME_HISTORY_SIZE * barWidth;
		const Int graphX = TheDisplay->getWidth() - (Int)m_gameTimePosition.x - graphWidth;
		const Int graphY = nextLineY;
		const Real maxScale = 66.0f; // 66ms = top of graph (~15fps)

		// Background
		TheDisplay->drawFillRect(graphX, graphY, graphWidth, graphHeight, GameMakeColor(0, 0, 0, 128));

		// 33ms reference line (30fps target)
		Int refLineY = graphY + graphHeight - (Int)(33.0f / maxScale * graphHeight);
		TheDisplay->drawLine(graphX, refLineY, graphX + graphWidth, refLineY, 1.0f, GameMakeColor(255, 255, 255, 64));

		// Draw bars from oldest to newest
		for (Int i = 0; i < FRAME_HISTORY_SIZE; ++i)
		{
			Int idx = (m_frameTimeHistoryIndex + i) % FRAME_HISTORY_SIZE;
			Real ft = m_frameTimeHistory[idx];
			if (ft <= 0.0f) continue;

			Int barH = (Int)(ft / maxScale * graphHeight);
			if (barH > graphHeight) barH = graphHeight;
			if (barH < 1) barH = 1;

			// Color by severity: green=smooth, yellow=slow, red=choppy
			UnsignedInt barColor;
			if (ft <= 33.0f)
				barColor = GameMakeColor(0, 200, 0, 200);      // green: 30fps+
			else if (ft <= 50.0f)
				barColor = GameMakeColor(255, 200, 0, 200);     // yellow: 20-30fps
			else
				barColor = GameMakeColor(255, 50, 50, 200);     // red: <20fps

			Int barX = graphX + i * barWidth;
			Int barY = graphY + graphHeight - barH;
			TheDisplay->drawFillRect(barX, barY, barWidth, barH, barColor);
		}
	}
}
