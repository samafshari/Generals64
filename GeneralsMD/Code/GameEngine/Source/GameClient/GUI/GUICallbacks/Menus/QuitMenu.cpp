...
static void exitQuitMenu()
{
  // destroy the quit menu
	...
	// Telemetry: flush any buffered events with a terminal game_exited
	// before the engine tears down state.
	if (TheGameTelemetry)
		TheGameTelemetry->onGameExited();
...
static void surrenderQuitMenu()
{
  // destroy the quit menu
	...
	// Telemetry: mark this terminal event as a surrender, distinct from a
	// plain exit or an elimination. Fires before the self-destruct message
	// so the flush lands while the network is still up.
	if (TheGameTelemetry)
		TheGameTelemetry->onGameSurrendered();
...
void ToggleQuitMenu()
{
	...
	else
	{
		...
		// of QuitMenuParent are not hidden when re-opening the menu. The transition
		// system only manages visibility of windows listed in the transition group
		// (parent + buttons). WinLoad has no transition entry, so if it ever ends up
		// with WIN_STATUS_HIDDEN set (e.g. from a prior layout hide), it will stay
		// invisible because no transition clears it.
		{
			GameWindow *parentWin = quitMenuLayout->getFirstWindow();
			if (parentWin)
			{
				for (GameWindow *child = parentWin->winGetChild(); child; child = child->winGetNext())
					child->winHide(FALSE);
			}
		}
