...
void ControlBar::onDrawableSelected( Drawable *draw )
{
	...
	// [GUARD-DEBUG] file log (debug builds only - fopen-per-select is expensive)
	{
		FILE *gdbg = fopen("guard_debug.log", "a");
		if (gdbg)
		{
			fprintf(gdbg, "onDrawableSelected frame=%d  draw=%s  selectCount=%d  hadPending=%d\n",
				TheGameLogic ? TheGameLogic->getFrame() : -1,
				(draw && draw->getTemplate()) ? draw->getTemplate()->getName().str() : "(null)",
				TheInGameUI ? TheInGameUI->getSelectCount() : -1,
				(TheInGameUI && TheInGameUI->getGUICommand()) ? 1 : 0);
			fclose(gdbg);
		}
	}
#endif
...
{
#if defined(DEBUG_LOGGING)
	// [GUARD-DEBUG] file log (debug builds only - fopen-per-deselect is expensive)
	{
		FILE *gdbg = fopen("guard_debug.log", "a");
		if (gdbg)
		{
			fprintf(gdbg, "onDrawableDeselected frame=%d  draw=%s  selectCount=%d  hadPending=%d\n",
				TheGameLogic ? TheGameLogic->getFrame() : -1,
				(draw && draw->getTemplate()) ? draw->getTemplate()->getName().str() : "(null)",
				TheInGameUI ? TheInGameUI->getSelectCount() : -1,
				(TheInGameUI && TheInGameUI->getGUICommand()) ? 1 : 0);
