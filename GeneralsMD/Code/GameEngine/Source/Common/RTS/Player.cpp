...
void Player::initFromDict(const Dict* d)
{
	...
	// Cosmetic shader effect id. GameLogic stuffs this from GameSlot.m_shaderId.
	// 0 = stock. Live overrides from RELAY_TYPE_COSMETICS go through the
	// CosmeticsCache at render time; this field is the lobby-time snapshot
	// used as a fallback when the cache is cold (e.g. single-player).
	Int sid = d->getInt(TheKey_playerShaderId, &exists);
	if (exists)
		m_shaderId = sid;
...
Bool Player::canBuildMoreOfType( const ThingTemplate *whatToBuild ) const
{
  ...
  {
    // For templates whose cap comes from the superweapon-restriction dropdown
    // (DeterminedBySuperweaponRestriction), a host-configured value of 0 means
    // "superweapons disabled" — deny construction. For all other templates, a
    // 0 cap continues to mean "no cap" (the classic INI-era semantic for
    // m_maxSimultaneousOfType left at its default).
		...
		{
			{
				...
				WEAPONBONUSCONDITION_SOLO_HUMAN_INSANE,
				WEAPONBONUSCONDITION_SOLO_HUMAN_NIGHTMARE
			},
			{
				...
				WEAPONBONUSCONDITION_SOLO_AI_INSANE,
				WEAPONBONUSCONDITION_SOLO_AI_NIGHTMARE
			}
		};
		if (apply)
