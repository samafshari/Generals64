...
GlobalData::GlobalData()
{
  ...
  m_showAudioLocations = FALSE;
	...
	m_useTerrainTessellation = FALSE;
	m_useFpsLimit = FALSE;
	...
	// Default ON so buildings/units drop shadows on terrain out of the box.
	// Was FALSE in the original (only enabled under "very high" LOD), which
	// silently suppressed all projected shadow decals on lower LOD presets.
...
//	m_languageFilterPref = false;
	...
	// Recompute run-ahead 10x/sec instead of 2x/sec so the dynamic adjuster
	// reacts to actual measured latency within ~100 ms of game start instead
	// of ~500 ms — directly cuts the early-game "feels laggy until it warms
	// up" window. The cost is one extra NetRunAheadCommandMsg send per host
	// every 100 ms, which is negligible.
	m_networkRunAheadMetricsTime = 100;
...
void GlobalData::parseGameDataDefinition( INI* ini )
{
	...
	// Read windowed/fullscreen preference from Options.ini.
	// Command line flags (-win / -fullscreen) parsed later will override this.
