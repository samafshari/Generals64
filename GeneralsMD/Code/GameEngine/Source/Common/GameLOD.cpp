...
//	m_useEmissiveNightMaterials=TRUE;
	...
	if (TheGlobalData)
	{
		...
		// -------------------------------------------------------------------
		// HARDCODED MAX QUALITY OVERRIDE
		// -------------------------------------------------------------------
		// Regardless of which LOD preset was just applied (Low/Medium/High/
		// VeryHigh/Custom) or what the user saved in their Options.ini, force
		// every graphics flag to its highest-quality value. The options menu
		// checkboxes still write to their respective fields but the next call
		// back through here resets everything. This is the single chokepoint
		// that all graphics LOD changes flow through, so overriding here is
		// sufficient to lock quality at max across the whole engine.
		TheWritableGlobalData->m_maxParticleCount        = 5000;
		TheWritableGlobalData->m_useShadowVolumes        = TRUE;
		TheWritableGlobalData->m_useShadowDecals         = TRUE;
		TheWritableGlobalData->m_textureReductionFactor  = 0;      // no reduction
		...
		TheWritableGlobalData->m_maxTankTrackEdges       = 100;
		TheWritableGlobalData->m_maxTankTrackOpaqueEdges = 25;
		TheWritableGlobalData->m_maxTankTrackFadeDelay   = 60000;
		TheWritableGlobalData->m_useTreeSway             = TRUE;
		TheWritableGlobalData->m_useDrawModuleLOD        = FALSE;  // extra anims ON, scaffolds ON
		TheWritableGlobalData->m_useHeatEffects          = TRUE;
		TheWritableGlobalData->m_enableDynamicLOD        = FALSE;  // never auto-downgrade
...
void GameLODManager::applyDynamicLODLevel(DynamicGameLODLevel level)
{
	...
	// Dynamically scale the particle count cap when FPS drops.
	// This aggressively reduces particles at lower LOD levels to maintain frame rate.
	if (TheWritableGlobalData && m_currentStaticLOD != STATIC_GAME_LOD_UNKNOWN)
	{
		static const Real particleScales[] = { 0.25f, 0.50f, 0.75f, 1.0f }; // Low, Medium, High, VeryHigh
		Int baseCount = m_staticGameLODInfo[m_currentStaticLOD].m_maxParticleCount;
		TheWritableGlobalData->m_maxParticleCount = (Int)(baseCount * particleScales[level]);
	}
}
