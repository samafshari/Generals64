	...
	CurMotionMode(BASE_POSE)
{
  ...
  ModeAnim.Motion=nullptr;
	...
	ModeAnim.LastAdvancedLogicFrame=0xFFFFFFFF;
 ...
 *=============================================================================================*/
// Track the logic frame to ensure animation advances only once per logic tick.
// Multiple render frames may occur per logic tick — only the first should advance.
static unsigned int s_lastAnimLogicFrame = 0xFFFFFFFF;
static unsigned int s_currentLogicFrame = 0;

void Animatable3DObjClass::SetRenderFreeze(bool freeze) { (void)freeze; }
bool Animatable3DObjClass::GetRenderFreeze() { return false; }

void Animatable3DObjClass::SetLogicFrame(unsigned int frame) { s_currentLogicFrame = frame; }

void Animatable3DObjClass::Single_Anim_Progress ()
{
	WWASSERT(CurMotionMode == SINGLE_ANIM);

	// Each object tracks its own last-advanced logic frame.
	// This ensures each object advances exactly once per logic tick,
	// regardless of how many render frames occur.
	if (ModeAnim.LastAdvancedLogicFrame == s_currentLogicFrame)
		return;
	ModeAnim.LastAdvancedLogicFrame = s_currentLogicFrame;
...
void Animatable3DObjClass::MarkAnimFrameComplete()
{
	s_lastAnimLogicFrame = s_currentLogicFrame;
}
