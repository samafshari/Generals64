...
class ParticleBufferClass : public RenderObjClass
{
	public:
		...
		*/
		...
		// from RenderObj...
      virtual bool	Is_Complete()		{ return IsEmitterDead && !NonNewNum && !NewNum; }
		...
		// D3D11 rendering support — update particle state without DX8 draw calls,
		// then expose data for external rendering.
		void UpdateStateForD3D11()
		{
			Get_New_Particles();
		...
		unsigned int Get_Start_Index() const { return Start; }
		unsigned int Get_End_Index() const { return End; }
		unsigned int Get_New_End_Index() const { return NewEnd; }
		int Get_Non_New_Count() const { return NonNewNum; }
		int Get_New_Count() const { return NewNum; }
		unsigned int Get_Max_Num() const { return MaxNum; }
		ShareBufferClass<Vector3>* Get_Positions() const { return Position[0]; }
		ShareBufferClass<Vector3>* Get_Previous_Positions() const { return Position[1]; }
		ShareBufferClass<Vector4>* Get_Diffuse_Array() const { return Diffuse; }
		ShareBufferClass<float>* Get_Sizes() const { return Size; }
		ShareBufferClass<uint8>* Get_Orientations() const { return Orientation; }
		ShareBufferClass<uint8>* Get_Frames() const { return Frame; }
		ShareBufferClass<float>* Get_UCoords() const { return UCoord; }
		int Get_Render_Mode_Value() const { return RenderMode; }
		float Get_Default_Frame_Value() const { return FrameKeyFrameValues ? FrameKeyFrameValues[0] : 0.0f; }
		float Get_Line_Width() const { return LineRenderer ? LineRenderer->Get_Width() : Get_Particle_Size(); }
