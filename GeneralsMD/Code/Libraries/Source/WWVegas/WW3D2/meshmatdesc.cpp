	...
	for (; pass<PassCount; pass++) {
		...
		*/
		...
		if ((DCGSource[pass] != VertexMaterialClass::MATERIAL) && (ColorArray[0] != nullptr) &&
			...
			for (int vidx=0; vidx<VertexCount; vidx++) {
				Vector4 diffuse=W3D_Convert_Color(diffuse_array[vidx]);
				Vector4 emissive=W3D_Convert_Color(emissive_array[vidx]);
				diffuse.X *= emissive.X;
				diffuse.Y *= emissive.Y;
				diffuse.Z *= emissive.Z;
				diffuse_array[vidx]=W3D_Convert_Color(diffuse);
			}
		}
	...
	*/
	for (pass=0; pass<PassCount; pass++) {
		...
		if ( (Shader[pass].Get_Primary_Gradient() == ShaderClass::GRADIENT_BUMPENVMAP) &&
			  (!W3DCaps::Is_Device_Initted() || W3DCaps::Support_Bump_Envmap() == false) )
		{
			kill_pass = true;
		}

		if ( (Shader[pass].Get_Primary_Gradient() == ShaderClass::GRADIENT_BUMPENVMAPLUMINANCE) &&
			  (!W3DCaps::Is_Device_Initted() || W3DCaps::Support_Bump_Envmap_Luminance() == false) )
		{
			kill_pass = true;
		}
		*/
...
{
	if (W3DCaps::Is_Device_Initted() && W3DCaps::Support_NPatches() && WW3D::Get_NPatches_Level()>1) return true;
