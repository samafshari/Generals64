...
{
	// DX11 data-only path: ignore legacy DX8 texture mappers for now.
	(void)vmat;
	(void)mapping0_arg_buffer;
	(void)mapping1_arg_buffer;
}
...
{
	// DX11 data-only path: material application is handled by ModelRenderer.
}

void VertexMaterialClass::Apply_Null()
{
	// DX11 data-only path: null material application is handled by ModelRenderer.
}
