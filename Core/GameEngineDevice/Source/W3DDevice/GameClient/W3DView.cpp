...
void W3DView::setCameraTransform()
{
	...
	// Camera constraints must be enforced in all builds, not just debug.
...
Drawable *W3DView::pickDrawable( const ICoord2D *screen, Bool forceAttack, PickType pickType )
{
	...
	while (window)
	{
		...
		{
			static int s_blockLog = 0;
			if (s_blockLog < 5) {
				char dbg[256];
				snprintf(dbg, sizeof(dbg), "pickDrawable BLOCKED by window status=0x%x at (%d,%d)\n",
					window->winGetStatus(), screen->x, screen->y);
...
Drawable *W3DView::pickDrawableIgnoreUI( const ICoord2D *screen, Bool forceAttack, PickType pickType )
{
	...
	Bool hit = W3DDisplay::m_3DScene->castRay( raytest, false, (Int)pickType );
	if( hit )
