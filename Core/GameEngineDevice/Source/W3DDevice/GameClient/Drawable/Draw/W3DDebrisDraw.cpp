  ...
  if (m_renderObject == nullptr && !name.isEmpty())
	{
		...
		m_renderObject = CreateRenderObjCompat(name.str(), getDrawable()->getScale(), hexColor, nullptr, nullptr);
		if (m_renderObject)
		{
			if (W3DDisplay::m_3DScene != nullptr)
...
void W3DDebrisDraw::setAnimNames(AsciiString initial, AsciiString flying, AsciiString final, const FXList* finalFX)
{
	...
	m_anims[INITIAL] = initial.isEmpty() || W3DDisplay::m_assetManager == nullptr ? nullptr : W3DDisplay::m_assetManager->Get_HAnim(initial.str());
	m_anims[FLYING] = flying.isEmpty() || W3DDisplay::m_assetManager == nullptr ? nullptr : W3DDisplay::m_assetManager->Get_HAnim(flying.str());
	...
	m_anims[FINAL] = final.isEmpty() || W3DDisplay::m_assetManager == nullptr ? nullptr : W3DDisplay::m_assetManager->Get_HAnim(final.str());
	m_state = 0;
	m_frames = 0;
	m_fxFinal = finalFX;
...
void W3DDebrisDraw::xfer( Xfer *xfer )
{
	...
	if( xfer->getXferMode() == XFER_LOAD )
		setAnimNames( m_animInitial, m_animFlying, m_animFinal, nullptr );
