...
{
	// Intentionally empty: trees are added to the terrain prop buffer in
	// reactToTransformChange() the first time the engine positions them.
	// A previous version of this file called addToTreeBuffer() FROM THE
	// DESTRUCTOR, which meant the tree never appeared in-game — only the
	// drawable's other modules (FX, topple) were left attached to the logical
	// Object, so optimised trees like USA01's TreeFir01B rendered as
	// floating particle effects (TreeBounceDust + embers) instead of meshes.
	// See the original ZH source: ~W3DTreeDraw is also empty there, and the
	// add happens in reactToTransformChange.
}

//-------------------------------------------------------------------------------------------------
// Called by the engine the first time the drawable's transform is set
// (i.e. when it has been positioned on the map). We use the m_treeAdded
// guard to add the tree to the terrain prop buffer exactly once, and the
// (0,0) early-out matches ZH's behaviour for "not yet positioned" props.
void W3DTreeDraw::reactToTransformChange( const Matrix3D* oldMtx, const Coord3D* oldPos, Real oldAngle )
{
	if (m_treeAdded)
		return;
