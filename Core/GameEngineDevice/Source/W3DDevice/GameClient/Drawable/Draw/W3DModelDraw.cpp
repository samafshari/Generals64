...
// Cached shell model trace flag - evaluated once per map load instead of per-call.
// The old code did strstr() on the map name on EVERY call (32 call sites in hot paths),
// and even with the early return, variadic argument setup couldn't be optimized away,
// causing 12.65% CPU overhead.
static bool s_shellModelTraceEnabledCached = false;
static bool s_shellModelTraceCacheValid = false;

static bool IsShellModelTraceEnabled()
{
	if (!s_shellModelTraceCacheValid)
	{
		s_shellModelTraceEnabledCached = TheGlobalData && std::strstr(TheGlobalData->m_mapName.str(), "ShellMap") != nullptr;
		s_shellModelTraceCacheValid = true;
	}
	return s_shellModelTraceEnabledCached;
}

static void AppendShellModelTraceImpl(const char *format, ...)
{
	FILE *f = fopen("shellmap_trace.log", "a");
	if (!f)
		return;
...
// Use a macro to completely eliminate argument evaluation and function call overhead
// when tracing is disabled. This avoids variadic argument setup on every call.
#define AppendShellModelTrace(...) do { if (s_shellModelTraceEnabledCached) AppendShellModelTraceImpl(__VA_ARGS__); } while(0)

static W3DAssetManager* GetDisplayAssetManager()
{
	if (W3DDisplay::m_assetManager == nullptr)
	{
		W3DDisplay::m_assetManager = EnsureWW3DAssetManagerInstance();
		AppendShellModelTrace("W3DModelDraw: lazy asset manager bind display=%p singleton=%p", W3DDisplay::m_assetManager, W3DDisplay::m_assetManager);
	}
	return W3DDisplay::m_assetManager;
}
...
HAnimClass* W3DAnimationInfo::getAnimHandle() const
{
	...
	if (m_handle == nullptr)
	{
		...
		W3DAssetManager* assetManager = GetDisplayAssetManager();
		m_handle = assetManager ? assetManager->Get_HAnim(m_name.str()) : nullptr;
		if (m_handle == nullptr)
			AppendShellModelTrace("W3DAnimationInfo: missing animation %s", m_name.str());
		if (m_handle)
		{
...
#else
	W3DAssetManager* assetManager = GetDisplayAssetManager();
	HAnimClass* handle = assetManager ? assetManager->Get_HAnim(m_name.str()) : nullptr;
	if (handle == nullptr)
		AppendShellModelTrace("W3DAnimationInfo: missing animation %s", m_name.str());
...
{
	AppendShellModelTrace("ModelConditionInfo: validateStuff begin model=%s transitionKey=%u robj=%p scale=%g",
		m_modelName.str(), m_transitionKey, robj, scale);
// srj sez: hm, this doesn't make sense; I think we really do need to validate transition states.
//	if (m_transition != NO_TRANSITION)
//		return;

	AppendShellModelTrace("ModelConditionInfo: loadAnimations begin model=%s", m_modelName.str());
	loadAnimations();
	AppendShellModelTrace("ModelConditionInfo: loadAnimations end model=%s", m_modelName.str());
	...
	AppendShellModelTrace("ModelConditionInfo: validateCachedBones begin model=%s", m_modelName.str());
	validateCachedBones(robj, scale);
	AppendShellModelTrace("ModelConditionInfo: validateCachedBones end model=%s", m_modelName.str());
	AppendShellModelTrace("ModelConditionInfo: validateTurretInfo begin model=%s", m_modelName.str());
	validateTurretInfo();
	AppendShellModelTrace("ModelConditionInfo: validateTurretInfo end model=%s", m_modelName.str());
	AppendShellModelTrace("ModelConditionInfo: validateWeaponBarrelInfo begin model=%s", m_modelName.str());
	validateWeaponBarrelInfo();
	AppendShellModelTrace("ModelConditionInfo: validateStuff end model=%s", m_modelName.str());
}
...
void ModelConditionInfo::validateCachedBones(RenderObjClass* robj, Real scale) const
{
	...
	if (robj == nullptr)
	{
		...
		robj = CreateRenderObjCompat(m_modelName.str(), scale, 0, nullptr, nullptr);
		if (!robj)
		{
			AppendShellModelTrace("ModelConditionInfo: missing model %s", m_modelName.str());
...
void W3DModelDrawModuleData::validateStuffForTimeAndWeather(const Drawable* draw, Bool night, Bool snowy) const
{
	...
	for (; c_it != m_conditionStates.end(); ++c_it)
	{
		...
		AppendShellModelTrace("W3DModelDrawModuleData: condition validate begin model=%s transitionKey=%u night=%d snowy=%d",
			c_it->m_modelName.str(), c_it->m_transitionKey, night, snowy);
		c_it->validateStuff(nullptr, draw->getScale(), m_extraPublicBones);
		AppendShellModelTrace("W3DModelDrawModuleData: condition validate end model=%s transitionKey=%u",
			c_it->m_modelName.str(), c_it->m_transitionKey);
	}
	...
	for (TransitionMap::iterator t_it = m_transitionMap.begin(); t_it != m_transitionMap.end(); ++t_it)
	{
		...
		if (a && b)
		{
			AppendShellModelTrace("W3DModelDrawModuleData: transition load begin key=%u", t_it->first);
		...
		// srj sez: hm, this doesn't make sense; I think we really do need to validate transition states.
			...
			AppendShellModelTrace("W3DModelDrawModuleData: transition load end key=%u", t_it->first);
		}
	}
}
...
{
	AppendShellModelTrace("W3DModelDraw: ctor begin thing=%p moduleData=%p", thing, moduleData);
	int i;
	...
	AppendShellModelTrace("W3DModelDraw: validateStuff begin");
	...
	AppendShellModelTrace("W3DModelDraw: validateStuff end");

	ModelConditionFlags emptyFlags;
	AppendShellModelTrace("W3DModelDraw: findBestInfo begin");
	const ModelConditionInfo* info = findBestInfo(emptyFlags);
	AppendShellModelTrace("W3DModelDraw: findBestInfo end info=%p", info);
	if (!info)
	{
	...
	AppendShellModelTrace("W3DModelDraw: getDrawable begin");
	Drawable* draw = getDrawable();
	AppendShellModelTrace("W3DModelDraw: getDrawable end draw=%p", draw);

  if ( draw )
  {
	  AppendShellModelTrace("W3DModelDraw: getObject begin");
	  Object* obj = draw->getObject();
	  AppendShellModelTrace("W3DModelDraw: getObject end obj=%p", obj);
	  if (obj)
	  {
	...
	AppendShellModelTrace("W3DModelDraw: setModelState begin info=%p", info);
	setModelState(info);
	AppendShellModelTrace("W3DModelDraw: ctor end");
}
...
void W3DModelDraw::setModelState(const ModelConditionInfo* newState)
{
	...
	//
	// PRESERVE BONE-ATTACHED PARTICLES ACROSS UNCHANGED-PARTICLE STATE CHANGES.
	// Many units (toxin tractor, anthrax tank, flame tank...) cycle their model
	// condition flags every weapon shot — FIRING_B → BETWEEN_FIRING_SHOTS_B →
	// FIRING_B again — and the matched ModelConditionInfo can change pointer
	// even when both states reference the *same* ParticleSysBone templates.
	// Tearing down and recreating the bone-attached system here forces every
	// new ground-aligned puddle particle to start at its first color keyframe
	// (R:0 G:0 B:0 for ToxinPuddleContinuous), which is invisible under
	// additive blending and takes ~30 frames to ramp up to peak green. Faster
	// than that, and the cycle produces a strobe — exactly the user-reported
	// "ground toxins disappear and flash" bug. Skip the destroy + recalc when
	// the new state's particle-bone vector is identical to the current one's.
	...
	// always stop particle systems now — UNLESS the new state's bone particle
	// templates are identical to the current state's, in which case keep them
	// alive so persistent ground effects don't strobe across state cycles.
	if (!bonesIdentical)
		...
		)
	{
		...
		{
			AppendShellModelTrace(
				"W3DModelDraw: setModelState begin model=%s scale=%g color=%08X",
				newState->m_modelName.str(),
				draw->getScale(),
				m_hexColor);
			m_renderObject = CreateRenderObjCompat(newState->m_modelName.str(), draw->getScale(), m_hexColor, nullptr, nullptr);
			AppendShellModelTrace(
				"W3DModelDraw: setModelState Create_Render_Obj end model=%s robj=%p",
				newState->m_modelName.str(),
				m_renderObject);
			if (m_renderObject == nullptr)
				AppendShellModelTrace("W3DModelDraw: missing model %s", newState->m_modelName.str());
		}
