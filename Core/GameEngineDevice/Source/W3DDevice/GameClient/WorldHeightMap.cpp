...
void WorldHeightMap::getUVForNdx(Int tileNdx, float *minU, float *minV, float *maxU, float*maxV)
{
	...
	*maxV/=m_terrainTexHeight;
	...
	// Inset by half a texel to prevent bilinear filtering from bleeding
	// across tile/quadrant boundaries in the atlas (causes checkerboard seams).
	Real halfTexelU = 0.5f / (Real)TEXTURE_WIDTH;
	Real halfTexelV = 0.5f / (Real)m_terrainTexHeight;
	*minU += halfTexelU;
	*minV += halfTexelV;
	*maxU -= halfTexelU;
	*maxV -= halfTexelV;
