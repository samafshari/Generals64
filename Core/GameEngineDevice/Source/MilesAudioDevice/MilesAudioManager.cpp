...
void MilesAudioManager::stopAudioEvent( AudioHandle handle )
{
	...
	for ( it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it ) {
		...
		if (audio->m_audioEventRTS->getPlayingHandle() == handle) {
			...
			notifyOfAudioCompletion(reinterpret_cast<uintptr_t>(audio->m_stream), PAT_Stream);
			break;
		}
	}
...
PlayingAudio *MilesAudioManager::findPlayingAudioFrom( uintptr_t audioCompleted, UnsignedInt flags )
{
	...
	if (flags == PAT_Sample) {
		HSAMPLE sample = reinterpret_cast<HSAMPLE>(audioCompleted);
		for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ++it) {
			playing = *it;
	...
	if (flags == PAT_3DSample) {
		H3DSAMPLE sample3D = reinterpret_cast<H3DSAMPLE>(audioCompleted);
		for (it = m_playing3DSounds.begin(); it != m_playing3DSounds.end(); ++it) {
			playing = *it;
	...
	if (flags == PAT_Stream) {
		HSTREAM stream = reinterpret_cast<HSTREAM>(audioCompleted);
		for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
			playing = *it;
...
void MilesAudioManager::initSamplePools()
{
	...
	for (i = 0; i < getAudioSettings()->m_sampleCount2D; ++i) {
		...
		if (sample) {
			AIL_init_sample(sample);
			AIL_set_sample_user_data(sample, 0, reinterpret_cast<void*>(static_cast<intptr_t>(i + 1)));
	...
	for (i = 0; i < getAudioSettings()->m_sampleCount3D; ++i) {
		...
		if (sample) {
			AIL_set_3D_user_data(sample, 0, reinterpret_cast<void*>(static_cast<intptr_t>(i + 1)));
...
{
	TheAudio->notifyOfAudioCompletion(reinterpret_cast<uintptr_t>(sampleCompleted), PAT_Sample);
}

//-------------------------------------------------------------------------------------------------
void AILCALLBACK set3DSampleCompleted( H3DSAMPLE sample3DCompleted )
{
	TheAudio->notifyOfAudioCompletion(reinterpret_cast<uintptr_t>(sample3DCompleted), PAT_3DSample);
}

//-------------------------------------------------------------------------------------------------
void AILCALLBACK setStreamCompleted( HSTREAM streamCompleted )
{
	TheAudio->notifyOfAudioCompletion(reinterpret_cast<uintptr_t>(streamCompleted), PAT_Stream);
}
