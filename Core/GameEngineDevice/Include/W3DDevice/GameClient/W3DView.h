...
public:
	...
	/// Raw drawable pick that SKIPS the "is a UI window over the cursor"
	/// check. Used by the inspector so shell-map / main-menu drawables can be
	/// inspected even though the menu UI windows cover the entire screen.
	/// Otherwise identical to pickDrawable.
	Drawable *pickDrawableIgnoreUI( const ICoord2D *screen, Bool forceAttack, PickType pickType );
  ...
  virtual WorldToScreenReturn worldToScreenTriReturn( const Coord3D *w, ICoord2D *s );	///< Transform world coordinate "w" into screen coordinate "s"
	...
	void drawPostOverlays();  ///< draw health bars, construction text, and other 2D overlays on top of 3D scene
