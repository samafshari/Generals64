#ifdef _M_IX86 // x86 crash dump code — disabled on x64
/*
...
#else
	symptr->Address = (DWORD)context->Rip;

	if (!IsBadCodePtr((FARPROC)context->Rip)) {
		if (_SymGetSymFromAddr != nullptr && _SymGetSymFromAddr (GetCurrentProcess(), (DWORD)context->Rip, &displacement, symptr)) {
			snprintf(scrap, ARRAY_SIZE(scrap), "Exception occurred at %016llX - %s + %08X\r\n",
				context->Rip, symptr->Name, displacement);
		} else {
		...
		DebugString ("Exception Handler: context->Rip is bad code pointer\n");
	}
#endif
...
#else
	Add_Txt("\r\nFloating point register dump not available on x64\r\n");
#endif
...
#else
	// x64: use RtlCaptureContext to get current register state
	CONTEXT localCtx;
	RtlCaptureContext(&localCtx);

	stack_frame.AddrPC.Mode = AddrModeFlat;
	stack_frame.AddrPC.Offset = localCtx.Rip;
	stack_frame.AddrStack.Mode = AddrModeFlat;
	stack_frame.AddrStack.Offset = localCtx.Rsp;
	stack_frame.AddrFrame.Mode = AddrModeFlat;
	stack_frame.AddrFrame.Offset = localCtx.Rbp;
#endif
...
#else
		if (_StackWalk(IMAGE_FILE_MACHINE_AMD64, GetCurrentProcess(), GetCurrentThread(), &stack_frame, nullptr, nullptr, _SymFunctionTableAccess, _SymGetModuleBase, nullptr)) {
#endif
