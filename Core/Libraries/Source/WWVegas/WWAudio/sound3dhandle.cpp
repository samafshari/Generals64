...
Sound3DHandleClass::Sound3DHandleClass ()	:
	SampleHandle (reinterpret_cast<H3DSAMPLE>(INVALID_MILES_HANDLE))
{
	return ;
}
...
Sound3DHandleClass::Initialize (SoundBufferClass *buffer)
{
	...
	if (SampleHandle != reinterpret_cast<H3DSAMPLE>(INVALID_MILES_HANDLE) && Buffer != nullptr) {
...
{
	if (SampleHandle != reinterpret_cast<H3DSAMPLE>(INVALID_MILES_HANDLE)) {
...
{
	WWASSERT (SampleHandle == reinterpret_cast<H3DSAMPLE>(INVALID_MILES_HANDLE));

	SampleHandle = reinterpret_cast<H3DSAMPLE>(handle);
	return ;
}
