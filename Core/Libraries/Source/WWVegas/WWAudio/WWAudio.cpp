...
WWAudioClass::WWAudioClass ()
	: m_Driver2D (nullptr),
	  ...
	  m_ReverbFilter (reinterpret_cast<HPROVIDER>(INVALID_MILES_HANDLE)),
	  m_UpdateTimer (-1),
...
WWAudioClass::Get_2D_Sample (const AudibleSoundClass &sound_obj)
{
	if (Is_OK_To_Give_Handle (sound_obj) == false) {
		return reinterpret_cast<HSAMPLE>(INVALID_MILES_HANDLE);
	}
	...
	HSAMPLE free_sample						= reinterpret_cast<HSAMPLE>(INVALID_MILES_HANDLE);
...
{
	if (Is_OK_To_Give_Handle (sound_obj) == false) {
		return reinterpret_cast<H3DSAMPLE>(INVALID_MILES_HANDLE);
	}
	...
	H3DSAMPLE free_sample					= reinterpret_cast<H3DSAMPLE>(INVALID_MILES_HANDLE);
