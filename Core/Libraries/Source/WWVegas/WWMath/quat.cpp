...
void __cdecl Fast_Slerp(Quaternion& res, const Quaternion & p,const Quaternion & q,float alpha)
{
	const float epsilon2 = WWMATH_EPSILON * WWMATH_EPSILON;
	Quaternion p_safe = p;
	Quaternion q_safe = q;
	if (!p_safe.Is_Valid() || p_safe.Length2() <= epsilon2) {
		p_safe.Make_Identity();
	} else {
		p_safe.Normalize();
	}
	if (!q_safe.Is_Valid() || q_safe.Length2() <= epsilon2) {
		q_safe = p_safe;
	} else {
		q_safe.Normalize();
	}
	alpha = WWMath::Clamp(alpha, 0.0f, 1.0f);
	...
	// cos theta = dot product of p and q
	cos_t = p_safe.X * q_safe.X + p_safe.Y * q_safe.Y + p_safe.Z * q_safe.Z + p_safe.W * q_safe.W;
	cos_t = WWMath::Clamp(cos_t, -1.0f, 1.0f);
	...
	if (1.0f - cos_t < epsilon2) {
		...
		if (WWMath::Fabs(sin_t) <= WWMATH_EPSILON) {
			beta = 1.0f - alpha;
		} else {
			oo_sin_t = 1.0f / sin_t;
	...
	res.X = beta*p_safe.X + alpha*q_safe.X;
	res.Y = beta*p_safe.Y + alpha*q_safe.Y;
	res.Z = beta*p_safe.Z + alpha*q_safe.Z;
	res.W = beta*p_safe.W + alpha*q_safe.W;

	if (!res.Is_Valid() || res.Length2() <= epsilon2) {
		res = p_safe;
	} else {
		res.Normalize();
		if (!res.Is_Valid()) {
