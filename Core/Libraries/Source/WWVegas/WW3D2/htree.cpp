...
{
	// NOTE: a previous "optimization" bailed out early when Get_Sync_Frame_Time() == 0
	// and the animation frame rate matched WWSyncPerSecond. That skipped the pivot
	// recomputation entirely, leaving stale bone transforms whenever Set_Animation
	// had just snapped the animation to a new frame (e.g. state transitions like
	// UNPACKING → DEPLOYED on the particle cannon). Always recompute — it's what
	// the original ZH code did, and skipping it breaks externally-driven frame jumps.
	...
	for (/*piv_idx*/; pivot < endpivot; pivot++,nodeMotion++) {
