...
//	virtual void					Set_Texture_Reduction_Factor(float trf);
	...
	// Cosmetic shader effect variant id for this object's owning player.
	// 0 = stock house-color shader. Non-zero = a player-activated cosmetic
	// effect that the D3D11 pixel shader applies on top of normal lighting
	// (Pulse, Rainbow, Shimmer, Chrome, etc). Mirrors the ObjectColor flow.
	void							Set_ObjectShaderId(int id) { ObjectShaderId=id; }
	int								Get_ObjectShaderId() const { return ObjectShaderId; }
...
protected:
	...
	int							ObjectShaderId;					// Cosmetic shader variant id (0 = stock).
