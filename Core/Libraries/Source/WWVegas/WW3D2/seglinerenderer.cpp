...
{
	Matrix4x4 view;
	// D3D11-removed: DX8Wrapper::Get_Transform(D3DTS_VIEW,view);

	Matrix4x4 identity(true);
// D3D11: 	// D3D11-removed: DX8Wrapper::Set_Transform(D3DTS_WORLD,identity);
// D3D11: 	// D3D11-removed: DX8Wrapper::Set_Transform(D3DTS_VIEW,identity);
...
// GENERALIZE FOR WHEN NO TEXTURE (DO NOT SET UV IN THESE CASES? NEED TO GENERALIZE FOR DIFFERENT TEXTURING MODES ANYWAY).
		...
		vArray[vidx].y = top.Y;
		vArray[vidx].z = top.Z;
		vArray[vidx].diffuse = W3D_Convert_Color(intersection[1][TOP_EDGE].RGBA);
		...
		vArray[vidx].diffuse = W3D_Convert_Color(intersection[1][BOTTOM_EDGE].RGBA);
		...
		for (; ; ) {
			...
			if (residual_top_points == 1 && residual_bottom_points == 1) {
				...
				vArray[vidx].y = top.Y;
				vArray[vidx].z = top.Z;
				vArray[vidx].diffuse = W3D_Convert_Color(intersection[top_int_idx][TOP_EDGE].RGBA);
				vArray[vidx].u1 = u_values[0] + uv_offset.X;
				...
				vArray[vidx].y = bottom.Y;
				vArray[vidx].z = bottom.Z;
				vArray[vidx].diffuse = W3D_Convert_Color(intersection[bottom_int_idx][BOTTOM_EDGE].RGBA);
				vArray[vidx].u1 = u_values[1] + uv_offset.X;
		...
		*/
		...
		unsigned int rgba;
		rgba=W3D_Convert_Color(Color,Opacity);
...
// D3D11: 		// D3D11-removed: DX8Wrapper::Set_Index_Buffer(ib_access,0);
// D3D11: 		// D3D11-removed: DX8Wrapper::Set_Vertex_Buffer(Verts);
// D3D11: 		// D3D11-removed: DX8Wrapper::Set_Material(mat);
// D3D11: 		// D3D11-removed: DX8Wrapper::Set_Texture(0,Texture);
// D3D11: 		// D3D11-removed: DX8Wrapper::Set_Shader(shader);

		if (sorting) {
			SortingRendererClass::Insert_Triangles(obj_sphere,0,tidx,0,vnum);
		} else {
// D3D11: 			// D3D11-removed: DX8Wrapper::Draw_Triangles(0,tidx,0,vnum);
		}
...
// D3D11: 	// D3D11-removed: DX8Wrapper::Set_Transform(D3DTS_VIEW,view);
