...
void LANAPI::handleRequestLocations( LANMessage *msg, UnsignedInt senderIP )
{
	...
	else
	{
		...
		if (m_currentGame)
		{
			if (m_currentGame->getSessionID(0) == m_sessionId)
			{
				LANMessage reply;
				fillInLANMessage( &reply );
	...
	LANPlayer *player = LookupPlayer(msg->sessionID);
	if (!player)
	{
		player = NEW LANPlayer;
		player->setIP(senderIP);
		player->setSessionID(msg->sessionID);
	}
	else
	{
		removePlayer(player);
...
void LANAPI::handleRequestJoin( LANMessage *msg, UnsignedInt senderIP )
{
	...
	// In relay mode, all lobby messages are broadcast. Filter by checking
	// if the join request targets our game (host session ID match).
	if (m_currentGame == nullptr || m_inLobby)
	{
		return;
	}
	if (msg->GameToJoin.gameSessionID != m_sessionId)
	{
		return; // Join request is for a different game, not ours.
	}
	LANMessage reply;
	fillInLANMessage( &reply );
	{
		if (m_currentGame->isGameInProgress())
		{
			...
			reply.GameNotJoined.gameSessionID = m_sessionId;
			reply.GameNotJoined.playerSessionID = msg->sessionID;
		...
		else
		{
			...
			for (player = 0; canJoin && player<MAX_SLOTS; ++player)
			{
				if (m_currentGame->getLANSlot(player)->isOpen())
				{
					...
					reply.GameJoined.gameSessionID = m_sessionId;
					reply.GameJoined.playerSessionID = msg->sessionID;

					LANGameSlot newSlot;
					newSlot.setState(SLOT_PLAYER, UnicodeString(msg->name));
					newSlot.setIP(senderIP);
					newSlot.setSessionID(msg->sessionID);
...
{
	// Check if this accept is for us by verifying the player session ID matches ours.
	if (msg->GameJoined.playerSessionID != m_sessionId)
	{
		return; // Accept is for a different player, not us.
	}
	{
		if (m_pendingAction == ACT_JOIN) // Are we trying to join?
		{
			...
			else
			{
				...
				slot.setIP(m_sessionId);  // Use session ID instead of local IP
				slot.setSessionID(m_sessionId);
...
{
	// Check if this deny is for us by verifying the player session ID matches ours.
	if (msg->GameNotJoined.playerSessionID != m_sessionId)
	{
		return; // Deny is for a different player, not us.
	}
	{
		if (m_pendingAction == ACT_JOIN) // Are we trying to join?
		{
...
void LANAPI::handleRequestGameLeave( LANMessage *msg, UnsignedInt senderIP )
{
	if (!m_inLobby && m_currentGame && !m_currentGame->isGameInProgress())
	{
		...
		{
			if (m_currentGame->getSessionID(player) == msg->sessionID)
			{
				if (player == 0)
				{
					OnHostLeave();
					...
					LANPlayer *lanPlayer = LookupPlayer(m_sessionId);
					if (!lanPlayer)
					{
						lanPlayer = NEW LANPlayer;
						lanPlayer->setIP(m_sessionId);
						lanPlayer->setSessionID(m_sessionId);
					}
					else
					{
...
void LANAPI::handleRequestLobbyLeave( LANMessage *msg, UnsignedInt senderIP )
{
	if (m_inLobby)
	{
		...
		while (player)
		{
			if (player->getSessionID() == msg->sessionID)
			{
				removePlayer(player);
...
void LANAPI::handleSetAccept( LANMessage *msg, UnsignedInt senderIP )
{
	if (!m_inLobby && m_currentGame && !m_currentGame->isGameInProgress())
	{
		...
		for (player = 0; player < MAX_SLOTS; ++player)
		{
			if (m_currentGame->getSessionID(player) == msg->sessionID)
			{
				OnAccept(msg->sessionID, msg->Accept.isAccepted);
				break;
			}
		}
	}
...
//	mapNameCRC.computeCRC(m_currentGame->getMap().str(), m_currentGame->getMap().getLength());
		...
		for (player = 0; player < MAX_SLOTS; ++player)
		{
			if (m_currentGame->getSessionID(player) == msg->sessionID)
			{
				OnHasMap(msg->sessionID, msg->MapStatus.hasMap);
				break;
			}
		}
	}
...
void LANAPI::handleChat( LANMessage *msg, UnsignedInt senderIP )
{
	...
	{
		LANPlayer *player;
		if((player=LookupPlayer(msg->sessionID)) != nullptr)
		{
	...
	else
	{
		...
		for (player = 0; player < MAX_SLOTS; ++player)
		{
			if (m_currentGame && m_currentGame->getSessionID(player) == msg->sessionID)
			{
...
{
	if (!m_inLobby && m_currentGame && m_currentGame->getSessionID(0) == msg->sessionID && !m_currentGame->isGameInProgress())
	{
		OnGameStart();
	}
}
...
void LANAPI::handleGameOptions( LANMessage *msg, UnsignedInt senderIP )
{
	if (!m_inLobby && m_currentGame && !m_currentGame->isGameInProgress())
	{
		...
		for (player = 0; player < MAX_SLOTS; ++player)
		{
			if (m_currentGame->getSessionID(player) == msg->sessionID)
			{
				OnGameOptions(msg->sessionID, player, AsciiString(msg->GameOptions.options));
				break;
			}
		}
	}
...
void LANAPI::handleInActive(LANMessage *msg, UnsignedInt senderIP) {
	...
	if (msg->sessionID != slot->getSessionID()) {
		return;
	}
