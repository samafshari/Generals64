...
//		DEBUG_LOG(("not sending packet, time = %d, m_lastFrameSent = %d, m_frameGrouping = %d", curtime, m_lastTimeSent, m_frameGrouping));
	...
	// Pool the NetPacket buffer across calls and across loop iterations: this
	// runs once per Connection per ~m_frameGrouping ms (≈28 ms at 70 Hz / 2-
	// frame run-ahead), and the host has one Connection per remote client, so
	// allocating and freeing a NetPacket here used to drive ~hundreds of
	// alloc/free pairs per second per host instance. The static is fine here
	// because the network update path is single-threaded — same pattern used
	// at sendNetCommandMsg() above.
	static NetPacket *s_pooledPacket = nullptr;
	if (s_pooledPacket == nullptr) {
		s_pooledPacket = newInstance(NetPacket);
	}

	while ((msg != nullptr) && couldQueue) {
		NetPacket *packet = s_pooledPacket;
		packet->reset(); // clear m_lastCommand etc. between uses
		...
		// Don't deleteInstance — packet is pooled and will be reused on the
		// next iteration of this loop and on the next call to doSend.
	}
