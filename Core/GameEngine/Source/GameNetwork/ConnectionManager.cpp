...
Bool ConnectionManager::isPlayerConnected( Int playerID )
{
	...
	// In C/S mode, non-host clients only own a Connection to the packet
	// router (host), so m_connections[peer] is always nullptr by design.
	// Use m_frameData[playerID] as the liveness signal instead — it is
	// allocated for every active human slot in both modes by parseUserList,
	// and gets set to null by disconnectPlayer when a slot is dropped.
	// Without this, DisconnectManager::updateDisconnectStatus skips every
	// peer slot on non-host clients and the disconnect-screen path never
	// fires, leaving the lockstep simulation permanently stalled when any
	// peer (not the host) closes their instance.
	if (m_clientServerMode && m_localSlot != (Int)m_packetRouterSlot)
		return (m_frameData[playerID] != nullptr && !m_frameData[playerID]->getIsQuitting());
...
Bool ConnectionManager::processNetCommand(NetCommandRef *ref) {
	...
	// "Is this player still in the game?" In P2P mesh mode every client holds
	// a direct Connection to every other player, so a null connection means
	// they left. In C/S mode, non-host clients ONLY hold a connection to the
	// packet router (host), and receive every other player's commands as relays
	// from the host — so a null m_connections[playerID] is the normal state and
	// must NOT be treated as "player left". Use m_frameData instead, which is
	// allocated for every active human slot in both modes (parseUserList).
	if (msg->getPlayerID() != m_localSlot) {
		if (m_clientServerMode) {
			if (m_frameData[msg->getPlayerID()] == nullptr) {
				return TRUE;
			}
		} else {
...
//
	...
	if ((m_localAddr == 0) || (m_localPort == 0)) {
		// In relay mode, we don't use a local UDP address/port — all traffic goes
		// through the TCP relay.  Skip this check so update() can proceed normally.
		if (!m_transport || !m_transport->isRelayMode()) {
...
void ConnectionManager::updateRunAhead(Int oldRunAhead, Int frameRate, Bool didSelfSlug, Int nextExecutionFrame) {
	...
	if ((lasttimesent == 0) || ((curTime - lasttimesent) > TheGlobalData->m_networkRunAheadMetricsTime)) {
		if (m_localSlot == m_packetRouterSlot) {
			...
			// CRITICAL: when we slugged ourselves (the cushion-based slowdown
			// in Network::timeForNewFrame fired this metrics window), report
			// the CONFIGURED rate, not the measured display FPS. Otherwise
			// the slug → measured-FPS-drops → adjuster clamps lower → next
			// frame is even slower → another slug → repeat creates a one-way
			// ratchet that locks the game at the floor (15 fps under MIN_LOGIC_FRAMES).
			// The user-visible symptom is "game starts at 30 fps, drops to 15 fps over
			// 15 minutes and red-bars". This is the original ZH fix that was commented
			// out — re-enabled because the lower MIN_RUNAHEAD makes the cushion-slug
			// fire constantly and this feedback loop dominates.
			if (didSelfSlug) {
			...
			// Floor the broadcast rate at the host's configured GFPS. Without this, any
			// transient measured-FPS dip (initial ramp-up, brief render hitch) ratchets the
			// lockstep rate down and — since render is gated by logic via timeForNewFrame —
			// locks the game at the floor for the rest of the match. The host's dropdown
			// choice is the authoritative target; the governor may exceed it but never
			// drop below.
			if (TheGameInfo) {
				const Int hostFloor = TheGameInfo->getGameFps();
				if (minFps < hostFloor)
					minFps = hostFloor;
			}
...
//				msg2->setID(GenerateNextCommandID());
			...
			// Cap at the host-configured ceiling (GameInfo::getGameFps).
			// CRITICAL: read the cap from TheGameInfo, NOT from TheNetwork->getFrameRate(),
			// because the network rate is itself the dynamically-adjusted value.
			// Reading from there created a one-way ratchet — once the rate dipped,
			// the cap dipped too and the rate could never recover. The host's choice
			// in the LAN options dropdown is the authoritative ceiling.
			Int gameRateCap = 30;
			if (TheGameInfo)
				gameRateCap = TheGameInfo->getGameFps();
			if (newMinFps > gameRateCap) {
				newMinFps = gameRateCap;
			}
			...
			// Same slug-aware reporting as the host path above. Without this
			// the cushion-based slowdown in Network::timeForNewFrame creates a
			// self-amplifying feedback loop that ratchets the network rate down
...
void ConnectionManager::setTransportRelayMode(Bool relay) {
	if (m_transport)
		m_transport->setRelayMode(relay);
}
...
void ConnectionManager::sendLocalCommandDirect(NetCommandMsg *msg, UnsignedByte relay) {
	...
	// In C/S mode a non-host client only owns a Connection to the host (the
	// packet router) — no direct connections to peer clients exist. The
	// original P2P-mesh code sent direct-send commands (LOADCOMPLETE, FILE,
	// PROGRESS, DISCONNECT*, etc.) to every recipient with a per-recipient
	// relay mask of `1 << i`, which assumes the recipient does NOT need to
	// re-broadcast. That assumption breaks in C/S mode: only the host receives
	// the message and, because the inbound relay mask is `1 << host_slot`, its
	// sendRemoteCommand finds nothing to forward — every other client is
	// silently starved. The most visible symptom is the 60-second
	// PROGRESS_COMPLETE_TIMEOUT freeze on game start, because every client's
	// LOADCOMPLETE only reaches the host.
	//
	// Fix: when we are a C/S client, route the direct-send through the host
	// with the FULL relay mask (minus our own slot, so the host doesn't bounce
	// it back). The host's processNetCommand → sendRemoteCommand path then
	// fans it out to the other clients using its full mesh of connections.
	if (m_clientServerMode &&
	    m_localSlot != (Int)m_packetRouterSlot &&
	    m_packetRouterSlot < MAX_SLOTS &&
	    m_connections[m_packetRouterSlot] != nullptr &&
	    !m_connections[m_packetRouterSlot]->isQuitting())
	{
		UnsignedByte temprelay = relay & ~(1 << m_localSlot);
		m_connections[m_packetRouterSlot]->sendNetCommandMsg(msg, temprelay);
		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("ConnectionManager::sendLocalCommandDirect - C/S client routing direct command %d of type %s to host (slot %d) with relay 0x%x", msg->getID(), GetNetCommandTypeAsString(msg->getNetCommandType()), m_packetRouterSlot, temprelay));
		msg->detach();
		return;
	}
...
//	}
	...
	if (slot == m_packetRouterSlot) {
		if (m_clientServerMode && slot != (Int)m_localSlot) {
			// Server disconnected in C/S mode — game is over, no router promotion
			DEBUG_LOG(("C/S mode: server (slot %d) disconnected, ending game for client", slot));
			disconnectLocalPlayer();
			return PLAYERLEAVECODE_PACKETROUTER;
		}
		Int index = 0;
...
void ConnectionManager::parseUserList(const GameInfo *game)
{
	...
	m_clientServerMode = game->isClientServerMode();
	if (m_disconnectManager) {
		m_disconnectManager->setClientServerMode(m_clientServerMode);
	}

	Int i;
	Int numUsers = 0;
	// Resolve local slot BEFORE the connection loop so C/S checks work correctly
	m_localSlot = game->getLocalSlotNum();
	DEBUG_LOG(("Local slot is %d, clientServerMode=%d", m_localSlot, m_clientServerMode));
	for (i=0; i<MAX_SLOTS; ++i)
	{
		...
		if (slot->isHuman())
		{
			...
			if (m_localSlot != i)
			{
				// In C/S mode, clients only connect to the server (packet router slot).
				// They still track FrameDataManager for all human slots (to receive relayed commands).
				Bool createConnection = TRUE;
				if (m_clientServerMode && m_localSlot != (Int)m_packetRouterSlot) {
					// We are a C/S client — only connect to the server
					createConnection = (i == (Int)m_packetRouterSlot);
				}
				...
				if (createConnection) {
					...
					DEBUG_LOG(("ConnectionManager::parseUserList - Remote user slot %d '%ls' at %d.%d.%d.%d:%d%s",
						i, slot->getName().str(),
						PRINTF_IP_AS_4_INTS(slot->getIP()), slot->getPort(),
						m_clientServerMode ? " (C/S server)" : ""));
				} else {
					DEBUG_LOG(("ConnectionManager::parseUserList - Remote user slot %d '%ls' (C/S: no direct connection, relayed via server)",
						i, slot->getName().str()));
				}
			...
			{
				DEBUG_LOG(("ConnectionManager::parseUserList - Local user slot %d '%ls' at %d.%d.%d.%d:%d",
					m_localSlot, slot->getName().str(),
					PRINTF_IP_AS_4_INTS(slot->getIP()), slot->getPort()));
