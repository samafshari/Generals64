...
NetRunAheadCommandMsg::NetRunAheadCommandMsg() : NetCommandMsg() {
	// Default to MIN_RUNAHEAD; the host's updateRunAhead path overwrites this
	// with the dynamically computed value before sending. The previous default
	// of 20 frames meant any RunAhead message that somehow shipped without an
	// explicit value would re-introduce ~666 ms of input lag.
