...
public:
	...
	// Push the host-chosen lockstep frame rate. Called from OnGameStart with
	// the value from GameInfo::getGameFps. The dynamic adjuster in
	// ConnectionManager::updateRunAhead may still revise this if it measures
	// a lower sustainable rate, but the cap-at-30 in that function has been
	// raised so the host's choice is normally what wins.
	...
	void initTransport();
	void setTransportRelayMode(Bool relay);
...
void Network::init()
{
	...
	m_lastFrame = 0;
	// Initial run-ahead = MIN_RUNAHEAD. Previously this was hardcoded to 30
	// frames (1 full second at 30 Hz), so every multiplayer game started with a
	// full second of input lag until the dynamic adjuster's first 500 ms tick
	// recomputed it down to ~MIN_RUNAHEAD on low-latency links. By starting at
	// the floor, gameplay feels instant from frame 1; if real network latency
	// is high, ConnectionManager::updateRunAhead will bump m_runAhead up
	// within the first metrics interval based on getMaximumLatency().
	m_runAhead = clamp<Int>(MIN_RUNAHEAD, MIN_RUNAHEAD, MAX_FRAMES_AHEAD/2);
	m_frameRate = 30;
...
#else
	// On non-Windows, use std::chrono nanosecond resolution and express frequency
	// in counts-per-second matching high_resolution_clock tick rate.
	m_perfCountFreq = std::chrono::high_resolution_clock::period::den
...
void Network::setTransportRelayMode(Bool relay) {
	DEBUG_ASSERTCRASH(m_conMgr != nullptr, ("Connection manager does not exist."));
	if (m_conMgr != nullptr) {
		m_conMgr->setTransportRelayMode(relay);
	}
}
...
//
	...
	// Transition from PREGAME to INGAME once frame 1 is reached.
	// This must happen before GetCommandsFromCommandList so that commands on
	// frame 1+ are not silently dropped. Originally the transition lived inside
	// processCommand() which only runs for non-transfer messages; if the command
	// list contained only transfer commands the transition would never fire.
	if (m_localStatus == NETLOCALSTATUS_PREGAME && TheGameLogic->getFrame() >= 1) {
		m_localStatus = NETLOCALSTATUS_INGAME;
		NetCommandList *netcmdlist = m_conMgr->getFrameCommandList(0);
		deleteInstance(netcmdlist);
	}
