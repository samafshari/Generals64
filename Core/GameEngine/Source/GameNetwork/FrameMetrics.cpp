...
void FrameMetrics::doPerFrameMetrics(UnsignedInt frame) {
	// Called exactly once per logic tick from ConnectionManager::processFrameTick.
	// Count invocations to derive logic FPS — NOT render FPS. The governor that
	// consumes m_averageFps adjusts the lockstep rate to match the slowest player's
	// logic throughput; if we fed it render FPS it would ratchet the lockstep
	// down to monitor refresh whenever vsync or any render-side cap was active.
	++m_logicFrameCountThisWindow;

	time_t curTime = timeGetTime();
	if (m_lastFpsTimeThing == 0) {
		m_lastFpsTimeThing = curTime;
	}
	const time_t elapsed = curTime - m_lastFpsTimeThing;
	if (elapsed >= 1000) {
		const Real measuredLogicFps = (Real)m_logicFrameCountThisWindow * 1000.0f / (Real)elapsed;
		m_averageFps -= m_fpsList[m_fpsListIndex] / TheGlobalData->m_networkFPSHistoryLength;
		m_fpsList[m_fpsListIndex] = measuredLogicFps;
		m_averageFps += measuredLogicFps / TheGlobalData->m_networkFPSHistoryLength;
		++m_fpsListIndex;
