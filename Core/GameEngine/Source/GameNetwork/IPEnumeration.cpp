...
EnumeratedIP * IPEnumeration::getAddresses()
{
	...
	if (!m_isWinsockInitialized)
	{
		...
		if ((LOBYTE(wsadata.wVersion) != 2) || (HIBYTE(wsadata.wVersion) !=2)) { WSACleanup(); return nullptr; }
#endif
...
#else
		DEBUG_LOG(("Failed call to gethostbyname"));
#endif
		return nullptr;
	}
...
AsciiString IPEnumeration::getMachineName()
{
	if (!m_isWinsockInitialized)
	{
		...
		int err = WSAStartup(verReq, &wsadata);
		if (err != 0) return "";
		if ((LOBYTE(wsadata.wVersion) != 2) || (HIBYTE(wsadata.wVersion) !=2)) { WSACleanup(); return ""; }
#endif
		m_isWinsockInitialized = true;
	}
