...
// MIN_RUNAHEAD is the floor on the lockstep look-ahead in logic frames. At 30
// Hz, the resulting input-lag floor is `MIN_RUNAHEAD * 33.3 ms`. The dynamic
// runahead computer in ConnectionManager::updateRunAhead clamps the per-game
// runahead to this floor, so this value is what actually decides loopback /
// LAN responsiveness — set it to 2 (66 ms) so loopback feels instant. The
// dynamic computer (`ceil(latency * 1.10 * fps)`) will auto-clamp HIGHER
// when real latency demands it, so this is safe for WAN play too.
...
Bool CommandRequiresDirectSend(NetCommandMsg *msg) {
	if ((msg->getNetCommandType() == NETCOMMANDTYPE_DISCONNECTVOTE) ||
			...
			(msg->getNetCommandType() == NETCOMMANDTYPE_DISCONNECTSCREENOFF) ||
			(msg->getNetCommandType() == NETCOMMANDTYPE_FRAMERESENDREQUEST) ||
			// DESTROYPLAYER (surrender / killPlayer-driven destruct) and
			// PLAYERLEAVE (exitGame → handleLocalPlayerLeaving, fired when
			// a terminal state like victory/defeat closes the game) must
			// use the direct-send path so sendLocalCommandDirect's C/S-mode
			// routing kicks in. Without this they go through sendLocalCommand
			// → packet-router relay, which assumes a P2P full mesh; in C/S
			// mode the host receives the command but never fans it out to
			// peers, so lockstep stalls waiting for an ack that will never
			// arrive (freeze on surrender / freeze on victory).
			(msg->getNetCommandType() == NETCOMMANDTYPE_DESTROYPLAYER) ||
			(msg->getNetCommandType() == NETCOMMANDTYPE_PLAYERLEAVE)) {
		return TRUE;
	}
