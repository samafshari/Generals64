...
FramePacer::FramePacer()
{
	...
	m_maxFPS = RenderFpsPreset::UncappedFpsValue; // Uncapped by default
...
{
	Int cap = isActualFramesPerSecondLimitEnabled() ? getFramesPerSecondLimit() : RenderFpsPreset::UncappedFpsValue;

	// Floor the render cap at the effective logic rate so render can never
	// throttle the simulation below the host / user pick. canUpdateGameLogic
	// advances at most one logic tick per render iteration (both in MP via
	// canUpdateNetworkGameLogic and in SP/skirmish via canUpdateRegularGameLogic
	// where the accumulator step is clamped to targetFrameTime via min(...)).
	// Without this floor, a 30 fps render cap (the GameData.ini default)
	// silently pins a 70 Hz skirmish sim to 30 Hz. See:
	//   memory/bug_mp_render_cap_locks_logic.md (MP symptom — same root cause)
	// MP path: TheNetwork->getFrameRate() is authoritative.
	// SP/skirmish path: FramePacer::getLogicTimeScaleFps() when scale enabled.
	if (TheNetwork != nullptr)
	{
		const Int netFps = TheNetwork->getFrameRate();
		if (netFps > 0 && cap < netFps)
			cap = netFps;
	}
	else if (isLogicTimeScaleEnabled())
	{
		const Int logicFps = getLogicTimeScaleFps();
		if (logicFps > 0 && cap < logicFps)
			cap = logicFps;
	}
