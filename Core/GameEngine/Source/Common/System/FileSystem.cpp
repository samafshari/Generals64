...
File*		FileSystem::openFile( const Char *filename, Int access, size_t bufferSize, FileInstance instance )
{
	...
	// Per-probe failures are NOT logged here. Higher-level loaders
	// (ImageCache::OpenTextureFile, W3DFileSystem, ...) probe multiple
	// candidate paths for a single logical asset — logging every probe
	// would flood the Inspector Log panel with expected misses. Each
	// loader logs its OWN terminal-failure message (see
	// ImageCache's "[Texture MISS]" line) after its whole fallback
	// chain has been exhausted, so the panel shows exactly the files
