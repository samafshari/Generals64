...
UnsignedInt INI::load( AsciiString filename, INILoadType loadType, Xfer *pXfer )
{
	...
	DEBUG_LOG(("INI: loading '%s'", filename.str()));
	...
	try
	{
		...
		while( m_endOfFile == FALSE )
		{
			...
			if( token )
			{
				...
				if (parse)
				{
					...
					try {
						(*parse)( this );
							...
							DEBUG_LOG(("INI PARSE ERROR: block '%s' in file '%s' at line %d", token, m_filename.str(), getLineNum()));
							char buff[1024];
				...
				{
					DEBUG_LOG(("INI UNKNOWN BLOCK: '%s' at line %d in file '%s'", token, getLineNum(), getFilename().str()));
