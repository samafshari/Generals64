...
void MultiPlayerLoadScreen::init( GameInfo *game )
{
	...
	for (i = 0; i < MAX_SLOTS; ++i)
	{
		...
		// revamp), NOT a palette index — see the comment on
		// GameSlot::setColor in GameInfo.h. The old call passed it
		// straight to MultiplayerSettings::getColor(int) which
		// interpreted it as an index into the 8-color preset list,
		// failed the bounds check, returned nullptr, and crashed when
		// the chained ->getColor() dereferenced the null. Use the
		// static resolveSlotColor() helper to convert the raw RGB int
		// into a renderer-ready ARGB Color (alpha=0xFF).
		Color houseColor = MultiplayerSettings::resolveSlotColor(slot->getApparentColor());
		// Tint the generic progress bar with the slot's house color.
		// The original ZH path looked up "LoadingBar_ProgressCenter%d"
		// where %d was a 0..7 palette index — with raw RGB ints that
		// lookup never matches anything, so every slot got the
		// untinted gray fallback. Use the same color-tint approach
		// the GENERALS path already uses.
...
void MultiPlayerLoadScreen::processProgress(Int playerId, Int percentage)
{
	...
	if(m_playerLookup[playerId] >= 0 && m_progressBars[m_playerLookup[playerId]])
...
//		DEBUG_ASSERTCRASH(m_playerFavoriteFactions[i], ("Can't initialize the StaticTextFavoriteFaction for the GameSpyLoadScreen loadscreen"));
		...
		// Raw-RGB slot color + bar tinting — see MultiPlayerLoadScreen::init
		// above for the why on both schema details.
		Color houseColor = MultiplayerSettings::resolveSlotColor(slot->getApparentColor());
		GadgetProgressBarSetEnabledBarColor(m_progressBars[netSlot], houseColor);
...
void MapTransferLoadScreen::init( GameInfo *game )
{
	...
	for (i = 0; i < MAX_SLOTS; ++i)
	{
		...
		if (!slot || !slot->isHuman())
			continue;
		// Raw-RGB slot color — see MultiPlayerLoadScreen::init above
		// for the why. resolveSlotColor pads alpha to 0xFF.
		Color houseColor = MultiplayerSettings::resolveSlotColor(slot->getApparentColor());
		GadgetProgressBarSetEnabledBarColor(m_progressBars[netSlot], houseColor );
