...
// ------------------------------------------------------------------------------------------------
/** Pre-age this particle to appear as if it was created 'frames' ago.
 *  Used for pre-warming particle trails on fast-moving emitters like missiles. */
// ------------------------------------------------------------------------------------------------
void Particle::preAge(UnsignedInt frames)
{
	if (frames == 0) return;

	// Back-date creation timestamp so alpha/color keyframes trigger correctly
	if (m_createTimestamp >= frames)
		m_createTimestamp -= frames;
	else
		m_createTimestamp = 0;

	// Reduce remaining lifetime
	if (m_lifetimeLeft > frames)
		m_lifetimeLeft -= frames;
	else
		m_lifetimeLeft = 1;
...
// ------------------------------------------------------------------------------------------------
Bool Particle::update(float gravityZ)
{
	// integrate acceleration (gravity applied directly) into velocity
	m_vel.x += m_accel.x;
	m_vel.y += m_accel.y;
	m_vel.z += m_accel.z + gravityZ;
...
Bool ParticleSystem::update( Int localPlayerIndex  )
{
	...
	//
	// Update all particles in the system (gravity folded directly into update)
	//
	Particle *p = m_systemParticlesHead;
	Particle *oldParticle;
	while (p)
	{
		if (p->update(m_gravity) == false)
		{
			oldParticle = p;
			p = p->m_systemNext;
...
{
	LIVE_PERF_SCOPE("ParticleSystemManager::update");
	TELEMETRY_SCOPE("Update", "ParticleSystemManager::update");
