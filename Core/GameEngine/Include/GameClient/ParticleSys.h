...
public:
	...
	Bool update(float gravityZ = 0.0f);		///< update this particle's behavior - return false if dead
	...
	UnsignedInt getLifetimeLeft() const { return m_lifetimeLeft; }
	UnsignedInt getCreateTimestamp() const { return m_createTimestamp; }

	UnsignedInt getPersonality() { return m_personality; };
	void setPersonality(UnsignedInt p) { m_personality = p; };

	void preAge(UnsignedInt frames);	///< pre-age this particle for pre-warming trails on moving emitters
