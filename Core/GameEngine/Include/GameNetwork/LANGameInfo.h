...
public:
	...
	/// Get the session ID of selected player or return 0
	UnsignedInt getSessionID( int who )
	{
		return m_LANSlot[who].getSessionID();
	}

	/// Set the session ID of the selected player
	void setSessionID( int who, UnsignedInt sessionID )
	{
		m_LANSlot[who].setSessionID(sessionID);
	}
