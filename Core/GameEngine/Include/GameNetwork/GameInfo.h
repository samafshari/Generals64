...
public:
	...
	// As of the launcher revamp, color is a raw 0x00RRGGBB integer
	// (NOT a palette index). -1 still means "random — pick at game
	// start". The launcher passes RGB via the -color command-line
	// arg, the in-game lobby's combo-box presets are translated to
	// their RGB values when picked, and every render site reads the
	// value as RGB via MultiplayerSettings::resolveSlotColor().
	void setColor( Int color ) { m_color = color; }
	Int getColor() const { return m_color; }

	// Optional cosmetic team-color shader id. 0 = default shader
	// (the engine's stock house-color replacement), non-zero = a
	// special shader bought/activated by the player. The render
	// path is the eventual consumer; for now we just round-trip
	// the value through the wire format and the player dictionary
	// so the marketplace UX can be built on top later.
	...
	void setSessionID( UnsignedInt sessionID ) { m_sessionID = sessionID; }
	UnsignedInt getSessionID() const { return m_sessionID; }
...
protected:
	...
	Bool m_hasMap;
	Bool m_isMuted;
	Int m_color;																			///< 0x00RRGGBB raw RGB, or -1 for random
	Int m_shaderId;																		///< 0 = default house-color shader, non-zero = activated cosmetic shader
	...
	UnsignedInt m_IP;																	///< Only valid for human players in LAN/WOL (used for transport addressing)
	UnsignedInt m_sessionID;													///< Only valid for human players in LAN/WOL (used for identification)
...
public:
  ...
  // Game logic / lockstep network frame rate, in Hz. The host picks this in
  // the LAN game options menu and it propagates to all clients via the game
  // options string. Drives both Network::m_frameRate (lockstep tick) and
  // FramePacer::setLogicTimeScaleFps (simulation step rate). Default 70 to
  // match the existing skirmish "fast game" setting; campaign and shell maps
  // ignore this and stay at 30 (see GameLogic::startNewGame).
  Int  getGameFps() const { return m_gameFps; }
  void setGameFps(Int fps) { m_gameFps = fps; }
