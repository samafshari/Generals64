...
#define DEBUG_FILENLINE			__FILE__ ":" DEBUG_TOKEN_IT(__LINE__)
	...
	// Optional sink hook for tooling (in-process inspector, log viewer,
	// telemetry forwarder). When set, every formatted log line is also
	// passed to the sink AFTER it's been written to file/console. The
	// sink runs on the same thread that called DebugLog and must not
	// re-enter the logger. Pass nullptr to clear.
	typedef void (*DebugLogSink)(const char *line);
